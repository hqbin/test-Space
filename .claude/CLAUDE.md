...
| **其他开发任务** | 后端、数据库、脚本、配置、文档等 | `development-workflow` |

> **调用方式**：判断任务类型后，使用 `Skill` 工具调用对应的 skill。如果任务同时涉及多个类型（如修 Bug 需要改前端），则依次调用多个 skill。调用 skills 前不需要问我确认，直接执行。
...
- `.agents/AGENTS.md` — AGENT 规范（编码约定、安全检查清单）
