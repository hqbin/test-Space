...
| 功能 | 说明 | 状态 |
|------|------|------|
| 自动日志开关 | 阈值告警触发时自动采集设备日志（logcat + dmesg + meminfo + top + getprop + df） | ✅ |
| 手动采集按钮 | 任意时刻手动触发日志采集 | ✅ |
...
| 4 | 自动日志/采集日志按钮 | 完全移除独立按钮；集成到 `startPolling()`/`stopPolling()` 生命周期，采集期间每 60s 自动保存一次 logcat+dmesg+meminfo+top+getprop+df |
...
> 用户经过长周期压测与多轮交互反馈后提出的 12+ 项细节打磨：时间偏差、AI 评估拖累、卡顿、Top N 交互、图表隐藏、固定颜色、按钮图标化、logcat/dmesg 实时性、HTML 报告同步等。本 Phase 集中归档这一批改动，并明确"单个应用内存无数据"是用户未启动目标应用所致（系统侧无 bug）。
...
| 14 | dmesg 实时输出 | 新增 `collectDmesg` 函数 + `dmesgTimer`（5 秒间隔），调用 `dmesg(serial)` 后追加写入 `dmesg.txt`，格式 `--- 2026-07-20 14:30:00 ---\n<output>\n`；`stopPolling` 中清理 timer |
| 15 | HTML 报告交互式图表同步 | `buildInteractiveChartsHtml` 重新设计：与主页面共用 colorMap、相同的图例选中/悬停交互逻辑、相同的 axisPointer 坐标值显示、相同的 tooltip 过滤规则；Top N CPU/TopApp 同样支持点击单条聚焦、再次点击恢复 |
