...
- 主内容区 `<main>`：`px-margin-page pt-3 box-border flex-1 overflow-y-auto overflow-x-hidden min-h-0 flex flex-col`
  ...
  - `min-h-0`：允许 flex 项收缩
- **`<keep-alive>`**：`router-view` 包裹 `keep-alive`，缓存 `NotesSpacePage` / `ScriptSpacePage` / `DeviceSpacePage` / `ApiSpacePage` / `SettingsPage` / `CaseSpacePage`，切换路由时组件不销毁重建，消除切换卡顿感。每个被缓存的页面组件必须通过 `defineOptions({ name: 'ComponentName' })` 声明 name 以匹配 keep-alive 的 `:include` 列表。
