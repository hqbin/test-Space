...
- **数据源**：`/proc/meminfo`（MemTotal/MemAvailable/SwapFree/ZRAM）、`dumpsys meminfo`（进程 PSS）、`/proc/stat`（CPU）、`df`（存储）
...
- **手动导出报告**（`exportReport()`）：
  ...
  - 两个 CSV（`perf_top_apps.csv` + `perf_top_cpu.csv`）：第一列为 Free RAM(KB)、Swap Free(KB)
