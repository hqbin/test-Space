<template>
  <div class="flex flex-col flex-1 min-h-0 -mx-margin-page pb-2 box-border select-none">
    ...
    <template v-else-if="record">
      <!-- Header -->
      <div class="flex items-center gap-3 mb-3 flex-shrink-0 ml-3 mr-3">
        <button class="glass-button px-2.5 py-1.5 rounded-lg text-[11px] flex items-center gap-1 select-none" @click="emit('back')">
          <span class="material-symbols-outlined text-[13px]">arrow_back</span>
          <span>{{ t('report.back') }}</span>
        </button>
        <div class="flex items-center gap-2 text-[12px] text-on-surface-variant/60">
        ...
        <span class="glass-panel px-2.5 py-0.5 rounded-full text-[11px] font-medium flex items-center gap-1"
          ...
          <span class="w-1.5 h-1.5 rounded-full" :class="record.status === 'done' ? 'bg-green-400' : record.status === 'failed' || record.status === 'aborted' ? 'bg-red-400' : 'bg-blue-400'"></span>
          {{ record.status }}
        </span>
        <div class="flex-1"></div>
        <button class="glass-button px-2.5 py-1.5 rounded-lg text-[11px] flex items-center gap-1 select-none" @click="exportHtml">
          <span class="material-symbols-outlined text-[13px]">file_download</span>
          <span>{{ t('report.exportHtml') }}</span>
        </button>
        <button class="glass-button px-2.5 py-1.5 rounded-lg text-[11px] flex items-center gap-1 select-none" @click="exportJson">
          <span class="material-symbols-outlined text-[13px]">data_object</span>
          <span>{{ t('report.exportJson') }}</span>
        </button>
      </div>

      <!-- Summary bar -->
      <div class="flex items-center gap-3 mb-3 flex-shrink-0 ml-3 mr-3 px-4 py-2 glass-panel rounded-2xl text-[11px] text-on-surface-variant/70">
        <span>{{ t('report.total') }}: <strong class="text-on-surface">{{ record.total }}</strong></span>
        <span class="w-1 h-1 rounded-full bg-on-surface-variant/20"></span>
        <span class="text-green-400">{{ t('report.passed') }}: <strong>{{ record.passed }}</strong></span>
        <span class="w-1 h-1 rounded-full bg-on-surface-variant/20"></span>
        <span class="text-red-400">{{ t('report.failed') }}: <strong>{{ record.failed }}</strong></span>
        <span class="w-1 h-1 rounded-full bg-on-surface-variant/20"></span>
        <span class="text-orange-400">{{ t('report.healed') }}: <strong>{{ record.healed }}</strong></span>
        <span class="w-1 h-1 rounded-full bg-on-surface-variant/20"></span>
        <span class="text-gray-400">{{ t('report.skipped') }}: <strong>{{ record.skipped }}</strong></span>
        <span class="flex-1"></span>
        <span class="text-on-surface-variant/40">{{ t('report.duration') }}: {{ record.durationMs }}ms</span>
      </div>

      <!-- Metric Cards Row -->
      <div class="grid grid-cols-4 gap-3 mb-3 flex-shrink-0 ml-3 mr-3">
        <div class="glass-panel rounded-2xl p-4 text-center">
          <div class="text-3xl font-bold text-on-surface">{{ record.total }}</div>
          <div class="text-[11px] text-on-surface-variant/60 mt-1">{{ t('report.total') }}</div>
        </div>
        <div class="glass-panel rounded-2xl p-4 text-center" style="background: linear-gradient(135deg, rgba(34,197,94,0.15), rgba(34,197,94,0.05));">
          <div class="text-3xl font-bold text-green-400">{{ record.passed }}</div>
          <div class="text-[11px] text-green-400/60 mt-1">{{ t('report.passed') }}</div>
        </div>
        <div class="glass-panel rounded-2xl p-4 text-center" style="background: linear-gradient(135deg, rgba(239,68,68,0.15), rgba(239,68,68,0.05));">
          <div class="text-3xl font-bold text-red-400">{{ record.failed }}</div>
          <div class="text-[11px] text-red-400/60 mt-1">{{ t('report.failed') }}</div>
        </div>
        <div class="glass-panel rounded-2xl p-4 text-center" style="background: linear-gradient(135deg, rgba(249,115,22,0.15), rgba(249,115,22,0.05));">
          <div class="text-3xl font-bold text-orange-400">{{ record.healed }}</div>
          <div class="text-[11px] text-orange-400/60 mt-1">{{ t('report.healed') }}</div>
        </div>
      </div>

      <!-- Status Distribution Bar -->
      <div v-if="record.total > 0" class="flex-shrink-0 ml-3 mr-3 mb-3 glass-panel rounded-2xl overflow-hidden h-8 flex">
      ...
      <!-- Tag Filter Row -->
      <div v-if="uniqueTags.length > 0" class="flex-shrink-0 ml-3 mr-3 mb-3 flex items-center gap-1.5 flex-wrap">
      ...
      <!-- Step Timeline -->
      <div class="flex-1 overflow-y-auto custom-scrollbar ml-3 mr-3">
        ...
        <div v-for="(step, idx) in filteredSteps" :key="step.id" class="relative pl-8 pb-3">
          ...
          <div class="glass-panel rounded-2xl overflow-hidden transition-all" :class="expandedSteps.has(step.id) ? 'bg-white/50' : 'bg-white/30 hover:bg-white/40'">
            ...
            <div v-if="expandedSteps.has(step.id)" class="px-4 pb-3 space-y-2 border-t border-white/10 pt-2">
              <!-- Screenshots -->
              <div v-if="step.screenshotBefore || step.screenshotAfter" class="grid grid-cols-2 gap-2">
              ...
              <div v-if="step.screenshotRef" class="glass-panel rounded-xl p-2 bg-white/20">
