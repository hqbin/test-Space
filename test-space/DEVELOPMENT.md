...
- 主内容区 `<main>`：`px-margin-page pt-3 box-border flex-1 overflow-y-auto overflow-x-hidden min-h-0 flex flex-col`
  ...
  - `min-h-0`：允许 flex 项收缩
- **`<keep-alive>`**：`router-view` 包裹 `keep-alive`，缓存 `NotesSpacePage` / `ScriptSpacePage` / `DeviceSpacePage` / `ApiSpacePage` / `SettingsPage`，切换路由时组件不销毁重建，消除切换卡顿感。每个被缓存的页面组件必须通过 `defineOptions({ name: 'ComponentName' })` 声明 name 以匹配 keep-alive 的 `:include` 列表。
...
**导航项**（按用户使用频率排序）：
Device → Notes → API → Scripts → Settings（Settings 固定在右侧）
...
- **双分页布局**（`:tabs="['常用命令', '其他命令']"`）：
  - **分页一：常用命令**
    - **日志采集（Logcat）**：
      ...
      - 开始/停止按钮
  - **日志采集（Diagnostic）**：
    - 一次性采集：计时显示采集时长，stop 时一次性收集 logcat + getprop + dumpsys + free + df + ps
  ...
  - **分页二：其他命令**
    ...
    - **密钥检查**（快捷指令下方）：点击「检查密钥」按钮检测机顶盒各种 key 的烧写情况，依次检测 HDCP 1.4、HDCP 2.2、MGKID、Widevine、Dolby 五项。检测流程：先执行 `adb root`，等待设备重连（轮询 `echo ok`，100ms 间隔，最多 30 次），然后依次执行各检测命令（顺序执行，避免 TEE 资源冲突）。检测结果以弹窗形式展示，每项显示支持/不支持状态（绿色/红色）+ 命令原始输出（可展开查看）。检测逻辑：HDCP 1.4/2.2/MGKID 使用 `tee_provision -qt 0x31/0x32/0xa2`，判断输出包含 `provisioned` 且不包含 `not provisioned`；Widevine 使用 `drminfo -d`，过滤含 `systemId` 的行，匹配 `systemId[=\s]+\d{5}`；Dolby 使用 `dolby_fw_dolbyms12`，判断输出包含 `exits!` 和 `done!`。支持时显示绿色文字（如"检测到设备支持HDCP1.4"），不支持时显示红色文字 + 警告信息（如"检测到设备不支持HDCP1.4    警告: HDCP1.4未烧写将导致无法播放受保护的高清内容"）。Dolby 支持时额外查询 `dumpsys media.audio_flinger --hal` 获取版本号（匹配 `build ms12 version:\s*(\d+)`，版本映射：2→2.4.2，3→2.8.2），原始输出仅显示匹配行。
...
#### 4.7.2 性能监控（`PerfMonitorPage.vue`）

**实现**：通过 ADB 实时采集设备性能数据（CPU/内存/进程 PSS），提供 ECharts 可视化图表、CSV 导出、HTML 交互式报告。

- **架构**：`/device-space/perf-monitor` 路由（keep-alive 缓存），`pollOnce()` 定时轮询 Rust 后端 `perf_get_snapshot`，`scheduleRender()` 300ms 防抖更新 ECharts，切页面时停止渲染但继续采集
- **数据源**：`/proc/meminfo`（MemTotal/MemAvailable/SwapFree/ZRAM）、`dumpsys meminfo`（进程 PSS）、`/proc/stat`（CPU）
- **CPU 采集修复**：原使用 `top -n 1 -b`（单次快照，显示进程生命周期累计均值，无法反映实时 CPU 突刺）。改为 `top -n 1 -d 1 -b`（1 秒间隔双次采样 delta，与 Android Studio Profiler/perfd 业界标准一致），每次采集增加约 1s 延迟但值正确反映瞬时 CPU 使用率。
> **注意**：改为 delta CPU 后，整体评估算法（`buildBuiltinAssessment`）中的阈值（峰值 >80%/>90%、CPU 突发占用等）仍然有效，因为瞬时值正确反映了真实 CPU 状况，之前用生命周期均值时反而会低估峰值。
- **HTML 报告增强**：`buildInteractiveChartsHtml` 新增 PSI 压力交互式折线图（6 条线：Mem/IO/CPU × Some/Full）和内存分布堆叠面积图（Free RAM/Cached/Buffers/Slab/KernelStack/PageTables/Other Used），按数据可用性条件渲染。`parsePerfCsv` 扩展解析全部 21 列 CSV 数据。`PerfCsvPoint` 接口新增压力/内存分布字段。字段命名统一：`buildInteractiveChartsHtml` 内部 `norm` 步骤同时兼容 store 的 `pressureMemSomeAvg10` 和 CSV 的 `pressureMemSome10` 两种命名。`exportReport`/`autoExportReport` 中的 `pts` 下采样后直接传递给新增图表。
- **Store**（`usePerfMonitorStore`）：维护 `history`（全局快照数组，最长 86400 点）、`processPssHistory`（Map<包名, PSS 时间序列>）、`processCpuHistory`（Map<包名, CPU 时间序列>）
- **实时图表**（`flex` 列布局，2 张 ECharts 折线图）：
  - **Top N 应用内存**：首两条线为 Free RAM（绿色，`memAvailKb`）和 Swap Free（蓝色，`swapFreeKb`），其后为按最新 PSS 排序的前 N 个应用的 PSS 曲线。右侧 legend 竖排滚动，显示实时数值 `(xxxM)`，超长包名在 220px 内自动换行（`overflow: 'breakAll'`）
  - **Top N 应用 CPU 使用率**：按最新 CPU 排序的前 N 个进程曲线，legend 显示实时数值 `(xx.x%)`
  - 两项图表共享：点击选中单个 series、再次点击恢复全部、emphasis 高亮、`replaceMerge` 增量更新、`bucketingDownsample`（最多 600 点）
- **开启/停止采集**：`startPolling()` 创建采集目录、初始化流式文件、启动 logcat 和定时器；`stopPolling()` 停止定时器、完成日志采集、自动执行 `autoExportReport()`
- **保存/加载会话**：序列化全部 store 状态为 JSON（文件名时间戳），通过原生对话框保存和加载
- **手动导出报告**（`exportReport()`）：
  - HTML 报告（`perf_report.html`，`max-width: 1400px`）：
    - 采集信息 + 汇总统计卡片
    - 整体评估（`buildOverallAssessment()`：CPU 趋势、内存趋势、内存泄漏检测、CPU 持续高占用等 6~8 条规则）
    - Top N 应用内存表格（含 Free RAM/Swap Free 的 min/max/avg 行）
    - Top N CPU 表格
    - 交互式 ECharts 图表（CDN 加载 echarts.js）：legend 竖排滚动 + `width: 340` `overflow: 'breakAll'` + 实时数值 `(xxxM)/(xx.x%)`，单击折线选中/恢复，resize 自适应
  - 三个 CSV（`perf_data.csv` + `perf_top_apps.csv` + `perf_top_cpu.csv`）：行格式 `Time,ProcessName,Value`，支持进程名逗号转义
- **流式文件**：采集期间自动保存 logcat 流、meminfo 快照、anr/tombstone 日志到采集目录，停止时自动按目录打包
  - **logcat**：Rust 端流式采集，`adb logcat -v time -b all` 全缓冲区，200MB 自动切分（文件名 `logcat_开始时间_结束时间.txt`），每 10 行 flush 防止数据丢失
  - **getprop.txt**：采集开始时写入，包含完整 `getprop` + `/proc/version` 输出
  - **versionInfo.txt**：采集开始时写入，仅包含 `ro.vendor.product.version` 属性值
  - **anr/tombstone**：开始和停止时各收集一次，合并到同一目录
  - **meminfo**：每 30s 采集一次，带时间戳
- **三路 CSV 实时流式写入**（每 30 点 flush 到磁盘）：
  - `perf_data.csv`：CPU/内存主数据（Time,CPU%,MemTotal,MemAvailable,SwapFree,ZRAM）
  - `perf_top_apps.csv`：Top N 进程内存（Time,ProcessName,PSS_KB）
  - `perf_top_cpu.csv`：Top N 进程 CPU（Time,ProcessName,CPU%）
- **报告数据来源**：`autoExportReport()` 和 `exportReport()` 均从 CSV 文件读取全量数据，支持超过内存上限（86400 点）的长期采集报告生成
- **Bucketing 下采样**：历史点超过 `MAX_CHART_POINTS=600`（图表）/ `DOWNSAMPLE_MAX=4000`（HTML 报告）时，使用 `bucketingDownsample()` 保留每个桶的极值（峰谷），避免降采样丢失关键信息
- **已移除文件**：`df.txt`、`proc_stat.txt`、`ps.txt`（无实际意义）
- **背景**：使用 Device Space 相同渐变
- **dmesg 环形缓冲区回绕修复**：`poll_dmesg`（`adb.rs`）新增 `last_line` 参数，当 `total_lines` 未增长但末行内容变化时（kernel ring buffer 回绕），返回全部 dmesg 数据，避免新内核消息永久丢失
- **logcat 进程守护**：新增 Rust 命令 `adb_logcat_is_alive`（`lib.rs`），`pollOnce()` 每 50 轮成功采集执行健康检查，检测到 logcat 子进程或读者线程异常退出时自动调用 `adb_logcat_start` 重启，确保通宵采集不中断
  - **线程健康追踪**：`LogcatChild` 结构体新增 `thread_alive: Arc<AtomicBool>`，读者线程退出时（无论因写错误、pipe 断开还是正常停止）自动更新为 `false`。`adb_logcat_is_alive` 同时检查子进程存活和 `thread_alive`，避免"读者线程已死但 ADB 进程存活"导致健康检查误判为正常
  - **Windows 文件锁容错**：读者线程写文件时若因被编辑器独占打开等原因导致 `write_all` 失败，不再直接 `break` 退出线程，改为跳过当前行 + 连续失败计数，连续 10 次失败才退出（给用户关闭编辑器恢复写入的时间）
  - 前端 `usePerfMonitor.ts` 新增 `logcatIsAlive()` 方法
  - Store `usePerfMonitorStore.ts` 新增 `dmesgLastLine` 状态跟踪
- **自适应退避**：`pollOnce()` 中移除连续 10 次错误自动停止机制，改为 ≥3 次后递增轮询间隔（最高退避到 30s），成功后自动恢复，logcat/dmesg 采集不受轮询错误影响
- **HTML 报告交互式图表修复**（`buildInteractiveChartsHtml` 内嵌脚本）：
  - **Reflow 时序**：ECharts 初始化从裸 `render()` 改为 `setTimeout(render, 50)`，确保浏览器在 charts-container `display:block` 后完成 layout reflow，容器尺寸不为 0
  - **Tooltip**：修复 `if (!selected) return ''` 导致未选中单条折线时 tooltip 全空的问题，改为无选中时显示全部系列值
  - **Legend 防递归**：`selectSeries` 中 `dispatchAction({ type: 'legendSelect' })` 会触发 `legendselectchanged` 事件 → 递归循环，新增 `legendGuard` 标志阻止重入
...
- **删除乐观 UI**：`doDeleteNote()` 的 UI 状态清理（从 notes 数组过滤、清空编辑器）在数据库调用**之前**执行，DB 删除改为 fire-and-forget（`.catch()` 日志），确保删除后界面立即响应无卡顿
- **初始化加载防卡顿**：`loadData()` 中的三个顺序 DB 查询（`loadNoteSpaces` / `loadNoteFolders` / `loadNoteList`）之间各插入一个 `yieldToMain()`，让浏览器在每次查询之间有机会渲染骨架屏，避免首次进入笔记页时的短暂冻结
...
- **实时自动保存**：编辑内容或名称变化后 1.5s 防抖写入 SQLite（`scheduleAutoSave` + `doAutoSave`），无需手动点击 Save 按钮。编辑器标题栏右侧显示保存状态（`scriptSaved` ref）：绿色圆点 + "已保存" / 灰色脉冲圆点 + "保存中..."。切换/新建/打开脚本时重置 `scriptSaved = true` 并清除待执行的自动保存定时器。脚本名称为空时跳过自动保存防止写入无效数据。Save 按钮保留用于手动强制保存，与自动保存互不冲突。
- **拖拽排序**：左侧脚本列表支持拖拽调整顺序，通过排序切换按钮进入「自定义」模式（`drag_indicator` 图标）后启用拖拽。由于 Tauri 配置了 `dragDropEnabled: false`，**不使用** HTML5 `draggable` API（会被 Tauri 拦截），改用 `mousedown` + `mousemove` + `mouseup` 模拟鼠标拖拽（`onItemMousedown` / `onWindowMousemove` / `onWindowMouseup`）。鼠标移动超过 4px 视为开始拖拽，通过 `document.elementFromPoint` + `data-script-id` attribute 识别目标行。松开后调用 `reorderScripts()` 乐观更新 `scripts.value` 并 `updateScriptSortOrders()` 持久化到 DB；失败时 reload 恢复。顺序持久化到 SQLite `scripts.sort_order` 列。排序模式循环：名称 ↑ → 名称 ↓ → 时间 → 自定义（拖拽） → 名称 ↑；排序模式持久化到 `localStorage('script-space:sort')`。
- **布局修复说明（Phase 25）**：脚本页面左侧列表底部分页器在首次进入时不显示（改变窗口大小后才正常）。根容器 `h-full` → `flex flex-col flex-1 min-h-0`，与 NotesSpacePage 同理——`<main>` 变为 `flex flex-col` 后，`h-full` 百分比高度在首次渲染时可能不生效，导致页面高度为 `auto`，左侧列表区域高度不足分页器不可见。改用 flexbox 自身 `flex: 1 1 0%` 撑满剩余高度后修复。
  - **额外 JS 修复**：`pageSize` 初始值 `999` → `10`，防止 `totalPages = Math.ceil(N/999) = 1` 导致分页器 `v-if` 隐藏；移除 `nextTick` 包装，在 `loadScriptList` 后直接调用 `updatePageSize()`，避免 async `onMounted` 中 `nextTick` 回调未按预期触发的问题脚本页面左侧列表底部分页器在首次进入时不显示（改变窗口大小后才正常）。根容器 `h-full` → `flex flex-col flex-1 min-h-0`，与 NotesSpacePage 同理——`<main>` 变为 `flex flex-col` 后，`h-full` 百分比高度在首次渲染时可能不生效，导致页面高度为 `auto`，左侧列表区域高度不足分页器不可见。改用 flexbox 自身 `flex: 1 1 0%` 撑满剩余高度后修复。
...
| `adb_shell` | `shell_command(serial, command)` | 在指定设备执行 shell 命令（捕获 stdout + stderr） | `adb -s <serial> shell <command>` |
...
用于 Diagnostic 采集（logcat + getprop + dumpsys + free + df + ps + ANR traces 打包为 ZIP）和文件夹下载。
...
.invoke_handler(tauri::generate_handler![
    ...
    adb_logcat_buffer_resize, adb_bugreport,
...
```typescript
const { listDevices, shell, installApk, uninstallApk, pushFile, pullFile, reboot, screenshot, logcatBufferResize, bugreport } = useAdb();
```
...
| Diagnostic 一次性采集 | 计时采集，stop 时一次性收集 logcat + getprop + dumpsys + free + df + ps，每个日志类型保存为独立文件，最后打包为 ZIP 压缩包 | ✅ |
| Bugreport 集成 | 调用 Rust 后端 `adb_bugreport` 命令（异步执行，不阻塞 UI） | ✅ |
| Rust 后端新增 2 命令 | `adb_logcat_buffer_resize`、`adb_bugreport` — adb.rs + lib.rs 注册 | ✅ |
| Rust 新增 `adb_start_screenrecord` | 启动录屏时异步 spawn 执行，不阻塞 UI | ✅ |
| Rust 新增 `create_zip` | 传入文件列表和输出路径，直接生成 ZIP 压缩包（基于 `zip` crate） | ✅ |
| `useAdb` 新增 2 方法 | `logcatBufferResize()`、`bugreport()` | ✅ |
...
| Device 页面翻译 | 连接栏、设备操作、快捷操作、应用管理、快捷指令、密钥检查、文件管理、屏幕镜像、遥控器、截图/录屏、所有 Toast 提示和命令执行消息 | ✅ |
...
- **密钥检查相关翻译 key**：`device.keyCheckResult`（弹窗标题）、`device.keyPassed`/`device.keyFailed`/`device.keyExecError`（状态标签）、`device.keySupported`/`device.keyNotSupported`（支持/不支持前缀）、`device.hdcp14Warning`/`device.hdcp22Warning`/`device.mgkidWarning`/`device.widevineWarning`/`device.dolbyWarning`（五项警告信息）
...
## 三十八、全页面代码健康审查（2026-07-07）

### 38.1 审查范围与方法

对所有页面进行完整的代码审查，检查维度：功能逻辑正确性、keep-alive 兼容性、内存泄漏（定时器/事件监听器/Tauri 事件订阅）、性能问题、边界情况与数据健壮性。

### 38.2 发现并修复的问题

#### 🔴 DeviceSpacePage — 5s 轮询在 keep-alive 后持续后台运行

**问题**：`DeviceSpacePage` 在 `onMounted` 里启动 5s 设备刷新 `setInterval`，但没有 `onDeactivated` 钩子，keep-alive 切走后定时器持续运行，每 5 秒仍在后台调用 `scanDevices()`，消耗 ADB 资源。

**修复**：新增 `onActivated` / `onDeactivated`。切走时 `clearInterval(autoRefreshId)` 停止轮询；切回来时重新启动。日志采集定时器（`logcatTimeoutId` / `diagTimeoutId` / `bootLogcatTimeoutId`）是用户主动操作，不在 deactivated 时停止，保持原有行为。

#### 🔴 ApiSpacePage — Tauri 事件监听器 keep-alive 后叠加注册

**问题**：`useApiProxy.init()` 在 `onMounted` 里调用，为 `proxy:request` / `proxy:response` / `proxy:breakpoint:request` 等 Tauri 事件注册监听器。`cleanup()` 在 `onUnmounted` 里调用，但 keep-alive 不触发 `onUnmounted`。结果：每次切回 API 页再 `onActivated`（原来没有）又调用 `init()`，导致同一事件有多个重复监听器，每条捕获请求被回调多次，界面上出现重复条目。

**修复**：新增 `onActivated` (重新 `init()`) 和 `onDeactivated` (`cleanup()`)。`onUnmounted` 保留作为最终兜底。

#### 🔴 CaseEditorPage — Ctrl+S 快捷键 keep-alive 后双重注册

**问题**：`document.addEventListener('keydown', onKeydown)` 在 `onMounted` 里注册，`onUnmounted` 里移除。keep-alive 后 `onMounted` 不再触发，但 `onUnmounted` 也不触发，监听器保留——这部分是对的。然而如果用户先进入编辑器（`onMounted`注册一次），切走再切回（keep-alive 不再触发 `onMounted`），此时只有一个监听器，行为正常。但若路由组件被 keep-alive exclude 然后重新 include，会导致重复注册。

**预防性修复**：新增 `onActivated` 注册 / `onDeactivated` 移除，确保快捷键只在页面活跃时生效，同时防止在其他页面误触发 Ctrl+S 保存。

#### 🟡 AppLayout — watch(route.path) 对 notes/script 页重复调用 loadAiConfig

**问题**：`watch(route.path)` 里先调 `ensureAiConfig()`（内部有 `aiConfigLoaded` 一次性标志），然后对 notes/script 页无条件再调一次 `loadAiConfig()`（每次切换都读 DB）。`ensureAiConfig` 的缓存设计被绕过。

**修复**：第二次 `loadAiConfig()` 改为只在 `aiConfigLoaded === true` 时（即已初始化后）执行，首次切换不会双读。

#### 🟡 database.ts — importAllData / importAllDataIncremental notes INSERT 缺 plain_text

**问题**：备份恢复的 notes INSERT 语句不含 `plain_text` 列，恢复后全部笔记的全文搜索失效。`repairEmptyPlainText` 的持久化标记也会阻止自动修复。

**修复**：两条 INSERT 都加上 `plain_text` 列（从备份数据读 `n.plainText ?? n.plain_text ?? ''`）；恢复完成后删除 `app_settings.plain_text_repaired` 标记并重置内存标志，确保下次启动重扫兜底。

#### 🟡 database.ts — repairEmptyPlainText 每次启动全表扫描 2 秒

**问题**：`repairEmptyPlainText` 在每次应用启动时调 `loadNoteList()` 触发，对 notes 表全表扫描（无索引，2 秒）。即使所有行已有 `plain_text`，每次启动仍扫描。

**修复**：完成修复后在 `app_settings` 写入 `plain_text_repaired = '1'`；下次启动先查这个标记（单行 PK 查询 < 1ms），存在则直接跳过扫描。

#### 🔵 NotesSpacePage — loadData 中 dataLoading 未用 finally

**问题**：`dataLoading.value = false` 放在 `try` 块内，若第一个 DB 查询抛异常，骨架屏永久不消失。

**修复**：改用 `finally { dataLoading.value = false }`。

#### 🔵 NotesSpacePage — onDeactivated 未清理 toastTimer

**问题**：`onDeactivated` 清理 `saveTimer` / `versionTimer` / `searchDebounce`，但漏掉 `toastTimer`，切走后 toast 定时器仍运行。

**修复**：补加 `if (toastTimer) clearTimeout(toastTimer)`。

### 38.3 修改文件汇总

| 文件 | 修改内容 |
|------|----------|
| `src/views/device-space/DeviceSpacePage.vue` | import 加 `onActivated`/`onDeactivated`；新增 `onActivated`（重启5s轮询）和 `onDeactivated`（停止5s轮询） |
| `src/views/api-space/ApiSpacePage.vue` | import 加 `onActivated`/`onDeactivated`；新增 `onActivated`（重新 init）和 `onDeactivated`（cleanup 事件监听器） |
| `src/views/case-space/editor/CaseEditorPage.vue` | import 加 `onActivated`/`onDeactivated`；新增 `onActivated`（注册 keydown）和 `onDeactivated`（移除 keydown），防止 keep-alive 下 Ctrl+S 双重注册或跨页误触发 |
| `src/layouts/AppLayout.vue` | `watch(route.path)` 里第二次 `loadAiConfig()` 改为仅在已初始化后执行，消除首次切换的双读 |
| `src/services/database.ts` | `repairEmptyPlainText` 完成后持久化标记；启动时先查标记直接跳过扫描；`importAllData` 和 `importAllDataIncremental` 的 notes INSERT 补加 `plain_text` 列；两个 import 函数结束后重置标记确保恢复后重扫 |
| `src/views/note-space/NotesSpacePage.vue` | `loadData` 改 `finally`；`onDeactivated` 补清 `toastTimer` |

### 38.4 keep-alive 兼容性规范（新增守则）

凡加入 `<keep-alive>` 缓存的页面组件，必须遵守以下规范：

1. **轮询定时器**：`onMounted` 启动，`onDeactivated` 停止，`onActivated` 恢复，`onUnmounted` 最终清理
2. **Tauri 事件监听器**（`listen()`）：`onMounted` 初始注册，`onDeactivated` 取消（`unlisten()`），`onActivated` 重新注册，`onUnmounted` 最终清理
3. **document 事件监听器**（全局快捷键等）：`onMounted` + `onActivated` 注册，`onDeactivated` + `onUnmounted` 移除
4. **数据刷新**：`onActivated` 中刷新可能在后台被修改的数据（如脚本列表、设备列表）
5. **组件 name**：每个被缓存的页面组件必须有 `defineOptions({ name: 'XxxPage' })`，且 name 必须出现在 `AppLayout` 的 `keep-alive :include` 列表中
...
该分数反映一个**快速迭代的工具类桌面应用**的实际水平：核心功能稳定、无数据安全漏洞、主要内存泄漏已清理、数据库层健壮。主要扣分来自文件规模过大和少量错误反馈缺失，不影响日常使用。

### 39.2 各维度评分

| 维度 | 得分 | 状态 |
|------|------|------|
| 编译 / 类型检查 | **98/100** | `vue-tsc --noEmit` 持续零错误 |
| 内存 / 资源管理 | **87/100** | keep-alive 四件套已覆盖所有缓存页面；本次补修拖拽中途切页的悬挂监听器 |
| 数据库层 | **80/100** | WAL + FTS5 + 增量/全量备份完整；`plain_text` 启动扫描已消除 |
| 错误处理 | **68/100** | 核心路径有 try-catch；`saveCurrentNote` 失败无用户反馈是主要缺口（本次部分改善：autosave 失败保留"保存中"状态） |
| 性能 | **80/100** | debounce、虚拟滚动、按需加载、keep-alive 缓存均到位；本次修复 autosave 每次重查全表的冗余 DB SELECT |
| 代码组织 | **52/100** | 三个超大文件是主要技术债（见下表）；无自动化测试 |
| 功能完整性 | **88/100** | 核心功能路径健全；删除笔记失败用户无感知为主要遗留 |

**主要超大文件（技术债，不影响运行）：**

| 文件 | 行数 | 说明 |
|------|------|------|
| `DeviceSpacePage.vue` | ~3000 | 设备管理、日志、镜像、文件管理混在一起 |
| `NotesSpacePage.vue` | ~2500 | 编辑器 + 目录树 + 导入导出 + 搜索 |
| `CaseEditorPage.vue` | ~2200 | 表格编辑 + 思维导图 + 列管理 |
| `database.ts` | ~1400 | 所有表的 CRUD + 迁移 + 备份恢复 |

### 39.3 本次新增修复（配合评估同步修改）

#### 性能：`doAutoSave` 消除冗余 DB 全表读

**问题**：自动保存触发 `db.saveScript()` 写入后，紧接着调用 `loadScriptList()` 从 DB 读全表——每 1.5s 写一次就读一次，纯粹浪费。

**修复**：保存成功后原地更新 `scripts.value` 中对应项（`scripts.value[idx] = {...}`），仅在脚本 id 不在列表中（第一次新建）时才调 `loadScriptList()`。DB 读取次数从"每次保存都读"降为"新脚本首次保存才读"。

#### 内存：`onDeactivated` 清理拖拽悬挂监听器

**问题**：鼠标拖拽排序时，`window.addEventListener('mousemove', ...)` 和 `window.addEventListener('mouseup', ...)` 注册在 window 上。若用户拖拽过程中切换页面（keep-alive deactivate），`mouseup` 永远不触发，两个监听器永久挂在 window 上，直到 `onUnmounted`。

**修复**：`onDeactivated` 里主动 `removeEventListener` 并重置所有拖拽状态。同时在 `onDeactivated` 里 `clearTimeout(autoSaveTimer)`，防止切走后定时器在后台触发写入。

### 39.4 已知遗留问题（不阻塞发布，后续迭代处理）

| 优先级 | 位置 | 问题 | 影响 |
|--------|------|------|------|
| 中 | `NotesSpacePage.saveCurrentNote` | 自动保存失败无用户反馈，`saved` 状态静默变回 true | 用户不知道内容未保存 |
| 中 | `NotesSpacePage.doDeleteNote` | 乐观 UI 后 DB 删除失败只打 log，用户无感知，重启后笔记恢复 | 行为与预期不符 |
| 低 | `ScriptSpacePage` 拖拽 | `onDeactivated` 已清理悬挂监听器，但未完成的排序不会保存（静默丢弃） | 用户感知拖拽中途切页操作丢失 |
| 低 | `CaseItem` 类型 | `[key: string]: any` 索引签名让所有字段都是 any，绕过了类型检查 | 类型安全缺口 |
| 低 | `database.ts` 行映射 | 35 处 `(row: any)` 裸 any 做 SELECT 结果映射 | 字段拼写错误在编译期不可见 |
| 低 | 无自动化测试 | 核心 DB 函数、工具函数均无单元测试 | 重构风险高 |

### 39.5 健康度提升路径

要从 72 分提升到 80+ 分，建议按顺序处理：

1. **saveCurrentNote 失败反馈**（2小时）：catch 块里把 `saved` 保持 false 并显示红色 toast
2. **database.ts 行映射类型化**（半天）：为每个 SELECT 结果定义 Row 接口，替换 `any[]`
3. **NotesSpacePage 拆分**（2天）：抽出 `NoteEditor.vue`、`NoteSidebar.vue`、`NoteExport.ts`
4. **核心函数单元测试**（1天）：`database.ts` CRUD、`repairEmptyPlainText`、`htmlToPlainText`
...
## Phase 43 — 性能监控（PerfMonitor）（已完成 ✅）

> 新增 `/device-space/perf-monitor` 路由，替代老旧 AMM 工具链，提供实时设备性能监控（CPU/内存/ZRAM/存储/进程 PSS 排行），支持数据采集、保存/加载会话、CSV 导出和阈值告警。

### 43.1 路由与入口

| 模块 | 说明 | 状态 |
|------|------|------|
| `src/router/index.ts` | 新增 `/device-space/perf-monitor` 路由 → `PerfMonitorPage.vue` | ✅ |
| `DeviceSpacePage.vue` | "其他命令"分页新增「性能监控」按钮跳转到 perf-monitor 页 | ✅ |
| `AppLayout.vue` | keep-alive `:include` 列表添加 `PerfMonitorPage` | ✅ |
| 背景渐变 | 使用 Device Space 相同的左中 secondary + 右中 primary 径向渐变 | ✅ |

### 43.2 Rust 后端采集命令

| 模块 | 说明 | 状态 |
|------|------|------|
| `src-tauri/src/adb.rs` | 新增 `get_perf_snapshot()` 函数：合并采集 meminfo + top + df + ps + dumpsys meminfo，解析为 `PerfSnapshot` 结构体（cpuPercent, memUsed/MemTotal, zramUsed/zramTotal, storageUsed/storageTotal, processes: Vec<ProcessInfo>） | ✅ |
| `src-tauri/src/lib.rs` | 注册 `perf_get_snapshot` 命令（`async` + `spawn_blocking`） | ✅ |

### 43.3 前端 Composable

| 模块 | 说明 | 状态 |
|------|------|------|
| `src/composables/usePerfMonitor.ts` | 封装 `perf_get_snapshot` invoke 调用，返回 `getSnapshot(serial): Promise<PerfSnapshot>` | ✅ |

### 43.4 Pinia Store（`usePerfMonitorStore`）

| 模块 | 说明 | 状态 |
|------|------|------|
| `src/store/usePerfMonitorStore.ts` | 新建 Pinia Store：`history` 队列（最多 120 点）、`realtime` 最新快照、`trackedPackages`、`alertEnabled` / `thresholdCpu` / `thresholdMem` | ✅ |
| `addPoint(snapshot)` | 推入 history 队列（超 120 移除最旧）；检测阈值告警（10s 防抖）；返回 `null` 或 alert 字符串；更新 `trackedPackages` 进程数据 | ✅ |
| `startPolling(serial)` / `stopPolling()` | `setInterval` 2s 轮询采集 + 生命周期控制 | ✅ |
| `getTrackedPackageHistory()` | 按 `trackedPackages` 过滤 history 中目标包 PSS 数据序列 | ✅ |
| `addTrackedPackage(pkg)` / `removeTrackedPackage(pkg)` | 追踪包管理接口 | ✅ |

### 43.5 页面 UI（`PerfMonitorPage.vue`）

#### 43.5.1 ECharts 双折线图

| 模块 | 说明 | 状态 |
|------|------|------|
| CPU 折线 | X 轴时间序列（HH:mm:ss），Y 轴 0-100%，主 Y 轴显示 CPU% | ✅ |
| 内存折线 | 双 Y 轴：左轴显示已用内存（GB），右轴显示剩余内存（GB） | ✅ |
| 实时更新 | `watch(store.history)` 触发 `setOption` 增量更新，不重建 chart | ✅ |
| 自适应 | `ResizeObserver` 监听容器尺寸变化调用 `chart.resize()` | ✅ |

#### 43.5.2 SVG 仪表盘

| 指标 | 说明 |
|------|------|
| CPU | 环形进度条 + 中央百分比数字，颜色区间：<50% 绿 / <80% 橙 / ≥80% 红 |
| 内存 | 同上，显示已用/总量（GB） |
| ZRAM | 同上，显示已用/总量（MB） |
| 存储 | 同上，显示已用/总量（GB） |
...
| 模块 | 说明 | 状态 |
|------|------|------|
| ECharts bar | Top 10 进程 PSS 水平柱状图，按 PSS 降序排列 | ✅ |
| 颜色编码 | 系统进程（android/system/com.android）灰色，用户进程紫色，追踪包高亮蓝色 + `*` 标记 | ✅ |

#### 43.5.4 控制栏

| 功能 | 说明 | 状态 |
|------|------|------|
| 开始/停止采集 | 按钮切换 `isPolling` 状态，调用 `store.startPolling(serial)` / `stopPolling()` | ✅ |
| 设备选择 | 复用 DeviceSpacePage 的设备列表（`useAdb` composable），下拉选择目标设备 | ✅ |
| 采集间隔指示 | 绿色脉冲点 + "采集中" / 灰色点 + "已停止" | ✅ |
| 阈值告警开关 | 启用后 CPU > thresholdCpu% 或内存 < thresholdMem GB 时自动 Toast 告警 | ✅ |
| 阈值配置 | `thresholdCpu`（默认 80%）和 `thresholdMem`（默认 0.5GB）输入框 | ✅ |

#### 43.5.5 保存/加载会话

| 功能 | 说明 | 状态 |
|------|------|------|
| 保存对话框 | Teleport 弹窗输入会话名称，调用 `savePerfSession()` 写入 SQLite `perf_sessions` 表 | ✅ |
| 加载对话框 | Teleport 弹窗列表展示已保存会话（名称 + 时间 + 采样数），点击加载恢复 history 数据到 Store | ✅ |
| 删除会话 | 加载对话框中每条记录右侧删除按钮，调用 `deletePerfSession()` | ✅ |

#### 43.5.6 CSV 导出

| 功能 | 说明 |
|------|------|
| `exportCsv()` | 生成 CSV 字符串（时间戳,CPU%,MemUsedGB,MemTotalGB,ZramUsedMB,ZramTotalMB,StorageUsedGB,StorageTotalGB 列头），通过 Blob URL + `<a>` 模拟点击下载 `perf_<timestamp>.csv` |

#### 43.5.7 阈值告警

| 功能 | 说明 | 状态 |
|------|------|------|
| `alertEnabled` 开关 | 控制告警是否启用（Store 中 `addPoint` 检测） | ✅ |
| CPU 超限 | CPU > thresholdCpu% 时返回 `"CPU 使用率过高: X%"` 告警字符串 | ✅ |
| 内存不足 | 剩余内存 < thresholdMem GB 时返回告警字符串 | ✅ |
| 10s 防抖 | `_lastAlertTime` 记录上次告警时间，10s 内不重复告警 | ✅ |
| Toast 展示 | 页面 `watch` Store 的 alert 返回值，调用 `showToast(alert, true)` 展示 | ✅ |

#### 43.5.8 自动日志采集

| 功能 | 说明 | 状态 |
|------|------|------|
| 自动日志开关 | 阈值告警触发时自动采集设备日志（logcat + meminfo + top + getprop + df） | ✅ |
| 手动采集按钮 | 任意时刻手动触发日志采集 | ✅ |
| 存放位置 | 保存到 `{appDataDir}/perf_logs/{timestamp}/`，兼容生产打包 exe 路径 | ✅ |
| 日志格式 | 6 类日志保存为独立 .txt 文件，方便查阅 | ✅ |
| 防阻塞 | `Promise.allSettled` 并行采集，不阻塞主轮询 | ✅ |

#### 43.5.9 图表与 UI 优化

| 功能 | 说明 | 状态 |
|------|------|------|
| ECharts ResizeObserver | CPU/内存趋势图随窗口大小自适应缩放 | ✅ |
| CPU 折线颜色 | 使用 secondary 紫色（`#7c5cfc`），渐变色面积填充 | ✅ |
| 内存双折线 | 已用（紫色）+ 剩余（绿色），双 Y 轴自适应单位（KB/MB/GB） | ✅ |
| 仪表盘颜色编码 | CPU <50% 绿 / <80% 橙 / ≥80% 红；内存 <70% 蓝 / <85% 橙 / ≥85% 红 | ✅ |
| 仪表盘显示格式 | 显示"已使用/总量"（如 `3.2/8.0 GB`） | ✅ |
| PSS 柱状图点击 | 点击柱状图弹出 Top 30 进程详情表格（#/名称/PID/PSS） | ✅ |
| 返回到按钮 | 按钮文本仅显示「返回」，无图标 | ✅ |
| 开始/暂停按钮 | 未采集时显示「开始」，采集中显示「暂停」 | ✅ |
| 会话列表删除按钮 | 加载对话框中 hover 显示删除按钮，支持逐条删除 | ✅ |

### 43.6 数据持久化

| 模块 | 说明 | 状态 |
|------|------|------|
| `perf_sessions` 表 | `id TEXT PK`, `name TEXT`, `data TEXT`(JSON 序列化的历史数据), `sample_count INTEGER`, `created_at TEXT` | ✅ |
| `src/services/database.ts` | `migrateInternal()` 中 `CREATE TABLE IF NOT EXISTS perf_sessions` | ✅ |
| CRUD 函数 | `savePerfSession()` / `listPerfSessions()` / `getPerfSession()` / `deletePerfSession()` | ✅ |
| `AppBackup` 扩展 | 新增 `perfSessions: PerfSession[]` 字段（版本 `1.7`） | ✅ |
| `exportAllData()` | 导出 `perf_sessions` 表数据到 `AppBackup.perfSessions` | ✅ |
| `importAllData()` | 恢复时删除 `perf_sessions` 表并重新插入 | ✅ |
| `validateBackup()` | 校验 `perfSessions` 字段 | ✅ |

### 43.7 i18n 翻译

| Key | zh | en |
|-----|----|----|
| `perf.title` | 性能监控 | Performance Monitor |
| `perf.start` | 开始采集 | Start |
| `perf.stop` | 停止采集 | Stop |
| `perf.collecting` | 采集中 | Collecting |
| `perf.stopped` | 已停止 | Stopped |
| `perf.cpu` | CPU | CPU |
| `perf.memory` | 内存 | Memory |
| `perf.zram` | ZRAM | ZRAM |
| `perf.storage` | 存储 | Storage |
| `perf.processes` | 进程排行 | Process Ranking |
| `perf.saveSession` | 保存会话 | Save Session |
| `perf.loadSession` | 加载会话 | Load Session |
| `perf.exportCsv` | 导出 CSV | Export CSV |
| `perf.sessionName` | 会话名称 | Session Name |
| `perf.sessionSaved` | 会话已保存 | Session saved |
| `perf.sessionLoaded` | 会话已加载 | Session loaded |
| `perf.sessionDeleted` | 会话已删除 | Session deleted |
| `perf.alertEnabled` | 告警 | Alerts |
| `perf.thresholdCpu` | CPU 阈值(%) | CPU Threshold(%) |
| `perf.thresholdMem` | 内存阈值(GB) | Mem Threshold(GB) |
| `perf.cpuHigh` | CPU 使用率过高: {value}% | High CPU usage: {value}% |
| `perf.memLow` | 可用内存不足: {value}GB | Low available memory: {value}GB |
| `perf.noDevice` | 请先选择设备 | Please select a device first |
| `perf.pssRanking` | PSS 排行 (Top 10) | PSS Ranking (Top 10) |
| `perf.trackedPackages` | 追踪包 | Tracked Packages |
| `perf.noHistory` | 暂无采集数据 | No data collected yet |

### 43.8 影响文件

| 文件 | 修改类型 |
|------|----------|
| `src-tauri/src/adb.rs` | 新增 `get_perf_snapshot()` + `PerfSnapshot` / `ProcessInfo` 结构体 |
| `src-tauri/src/lib.rs` | 注册 `perf_get_snapshot` 命令 |
| `src/composables/usePerfMonitor.ts` | 新增 composable |
| `src/stores/usePerfMonitorStore.ts` | 新增 Pinia Store |
| `src/views/device-space/PerfMonitorPage.vue` | 新增主页面（~600 行） |
| `src/services/database.ts` | 新增 `perf_sessions` 表 CRUD + 备份恢复集成 |
| `src/router/index.ts` | 新增路由 `/device-space/perf-monitor` |
| `src/layouts/AppLayout.vue` | keep-alive include 添加 `PerfMonitorPage` |
| `src/views/device-space/DeviceSpacePage.vue` | 新增「性能监控」按钮 |
| `src/composables/useI18n.ts` | 新增所有 `perf.*` 翻译键（中/英） |
...
## Phase 44.2 — PerfMonitor 第三次迭代：布局重构 + 进程趋势（已完成 ✅）

> 移除 PSS 排行面板，页面改为 2×2 四面板布局：内存趋势 / CPU 趋势（上行） + Top N 应用内存趋势 / 单个应用内存趋势（下行）。Top N 支持下拉选择 5/10/20/25/30，单个应用启动时自动检测前台应用并支持搜索切换。

### 变更清单

| # | 变更 | 说明 |
|---|------|------|
| 1 | 移除 PSS 排行 | 整个 PSS 面板及详情弹窗、进程详情表全部删除 |
| 2 | 四面板 2×2 布局 | 上行：内存趋势（双折线已用/剩余）+ CPU 趋势；下行左：Top N 应用内存趋势 + 下拉 N 选择器；下行右：单个应用内存趋势 + 搜索选择器 |
| 3 | Top N 应用内存 | 基于 `processPssHistory` 取最新 PSS 值倒序排列，可选择 5/10/20/25/30 条折线；每条折线显示进程短名，不同颜色区分 |
| 4 | 单个应用内存 | 启动时自动执行 `dumpsys window displays | grep mCurrentFocus` 检测前台应用包名并设为默认；输入框支持搜索过滤进程列表，点击选定后显示该进程 PSS 随时间变化的折线 |
| 5 | 导出报告 | 新增 Top N 应用内存 CSV（时间对齐的多列表）和单个应用内存 CSV（时间 + PSS 两列） |
| 6 | Store 精简 | 移除 `trackedPackages`/`processHistories`/`addTrackedPackage`/`removeTrackedPackage`；新增 `topAppCount`/`singleAppName` |
| 7 | i18n 精简 | 移除 `perf.processPss`/`perf.processTrend`/`perf.procName`/`perf.pss`/`perf.exportCsv` |

### 文件变更

| 文件 | 修改 | 说明 |
|------|------|------|
| `src/views/device-space/PerfMonitorPage.vue` | 重写 | 4 面板布局 + Top N 图表 + 单应用图表 + 前台应用检测 + 搜索选择器 + 导出报告增强 |
| `src/stores/usePerfMonitorStore.ts` | 重写 | 移除 `trackedPackages` 系列，新增 `topAppCount`/`singleAppName` |
| `src/composables/useI18n.ts` | 修改 | 移除 PSS 相关翻译键 |
...
## Phase 44.1 — PerfMonitor 二次修复（已完成 ✅）

> 基于第二轮用户反馈修复 5 个问题：清空历史后图表不恢复、导出 CSV 改为导出报告、图例点击切换折线显示、PSS 点击不响应 + 进程内存趋势图。

### 问题修复清单

| # | 问题 | 修复方式 |
|---|------|----------|
| 1 | 清空历史后图表全没了，再次开始/加载都不恢复 | `echarts.clear()` 销毁所有组件（grid/axes）；改为 `chart.setOption({ series: [{ data: [] }] })` 仅清空数据，保留图表配置 |
| 2 | 导出 CSV → 导出报告 | 改为生成自包含 HTML 报告：采集信息、汇总统计、进程 PSS 排行、完整 CSV 数据表格、日志引用；使用 Tauri dialog 选择保存路径 |
| 3 | 内存图例点击无反应、tooltip 颜色不一致 | 图例彩色圆点添加 `@click` → `echarts.dispatchAction({ type: 'legendToggleSelect' })` 切换折线显示/隐藏；tooltip 颜色自动继承 series lineStyle.color |
| 4 | CPU 趋势图也要支持隐藏/显示 | 在 CPU 标题栏添加紫色圆点 + "CPU" 文字，点击切换 |
| 5 | PSS 点击还是没反应 + 想看 Top 应用内存趋势 | 双层 div：底层 ECharts canvas + 上层透明 `z-10` overlay 拦截点击（不依赖 ECharts 事件）；新增 PSS 面板切换按钮「排行」/「趋势」：排行 = 当前快照柱状图，趋势 = Top 5 进程 PSS 折线图（随时间变化） |

### 文件变更

| 文件 | 修改类型 | 变更说明 |
|------|----------|----------|
| `src/views/device-space/PerfMonitorPage.vue` | 重写 | 修复 clearHistory、legend 点击切换、PSS 面板 overlay + 趋势图、导出报告 |
| `src/stores/usePerfMonitorStore.ts` | 修改 | 新增 `processPssHistory` Map 跟踪 Top 10 进程 PSS 历史 |
| `src/composables/useI18n.ts` | 修改 | 新增 `perf.exportReport`/`perf.processTrend` 键 |
...
## Phase 44 — PerfMonitor 修复与 UI 精简（已完成 ✅）

> 基于用户反馈修复 10 个问题：CPU 总是 0、PSS 点击无反应、按钮位置/翻译、CSV 导出改用 Tauri dialog、去掉 gauges/alert/log 按钮、内存 legend 移上方+颜色对齐、自动日志集成到 start/stop 生命周期。

### 问题修复清单

| # | 问题 | 修复方式 |
|---|------|----------|
| 1 | PSS 排行点击无反应 | ECharts `on('click')` → 容器 div `@click="showPssDetail"`；ECharts click 仅在有数据点时触发，无数据时无响应 |
| 2 | 开始按钮无翻译 | 硬编码中文「开始」「暂停」，不再通过 i18n key 渲染 |
| 3 | 开始按钮位置 | 移到「返回」按钮右侧，分隔线之后、状态指示器之前 |
| 4 | 自动日志/采集日志按钮 | 完全移除独立按钮；集成到 `startPolling()`/`stopPolling()` 生命周期，采集期间每 60s 自动保存一次 logcat+meminfo+top+getprop+df |
| 5 | CSV 导出 | Blob URL 下载 → Tauri `@tauri-apps/plugin-dialog` `save()` 弹出系统保存路径选择器 |
| 6 | Gauges 卡片行 | 整行移除（CPU/内存/ZRAM/存储 4 个仪表盘 + PSS 柱状图） |
| 7 | CPU 趋势一直显示 0 | Rust 解析增强：支持 `User 23%, System 5%` 格式（无 "cpu" 关键词）+ 兜底 `dumpsys cpuinfo` 解析 TOTAL 行 |
| 8 | 内存 legend 位置 & 颜色 | legend 从 ECharts 底部移到 HTML 标题栏（彩色圆点 + 文字），颜色使用实际折线色 `#7c5cfc`(已用) / `#22c55e`(剩余) |
| 9 | 告警功能按钮 | 完全移除（Store 中 `alertEnabled`/`thresholdCpu`/`thresholdMem`/`lastAlertTime` 一并删除） |
| 10 | 持续采集性能影响 | 见下方研究结论 |
...
| 数据源 | 行业认可度 | 说明 |
|--------|-----------|------|
| `dumpsys meminfo` | ✅ Google 官方推荐（Perfetto 文档、Android Developers、Qualcomm 文档） | Android Studio Profiler 也使用 dumpsys 作为数据源 |
| `cat /proc/meminfo` | ✅ Linux 标准接口，MemAvailable 是系统可用内存的最佳估算 | Qualcomm 文档推荐用于获取设备级别总内存 |
| `top` | ⚠️ 标准工具但精度有争议 | 输出格式因 Android 版本差异大，建议用 `dumpsys cpuinfo` 替代 |
| `dumpsys cpuinfo` | ✅ Digital.ai Testing 等商业工具使用 | 结果会除以 CPU 核心数，返回 0-100% |
| `df` | ✅ Linux 标准 filesystem 统计 | 轻量级，仅读取内核 filesystem 表 |
| `ps` | ✅ 标准进程列表 | 配合 `dumpsys meminfo` 可获取 PSS 信息 |

**结论**：采用的采集方案与业界的数字体验监控平台（Digital.ai Testing、Google Perfetto、Android Studio Profiler）一致，科学严谨。

#### 10. 持续采集对设备性能的影响

| 因素 | 影响程度 | 说明 |
|------|---------|------|
| ADB shell 进程创建 | 轻量 | 每次 `adb shell` 在 PC 端和 Android 端各创建一个 shell 进程，1-2s 间隔开销可接受 |
| `top -n 1 -b` | 中等 | 读取 `/proc/stat`，在部分设备上有额外 CPU 开销 |
| `dumpsys meminfo` | 轻-中等 | Java 系统服务调用，建议 2s+ 间隔 |
| `cat /proc/meminfo` | 极轻 | 纯内核内存读取 |
| 合并单次 ADB 调用 | ✅ **最佳实践** | 当前实现用 `;` 合并多命令为一次 `adb shell`，减少连接开销 |
| 推荐间隔 | 2-5s | 1s 间隔对老设备可能有影响，默认 2s 是合理选择 |

**结论**：2s 间隔、合并单次 ADB 调用的方案对设备性能影响极小，这是业界标准做法（StackOverflow 社区确认 ADB polling 是通用实践，Android Developers 文档中将其列为标准测试方法）。

### 文件变更

| 文件 | 修改类型 | 变更说明 |
|------|----------|----------|
| `src-tauri/src/adb.rs` | 修改 | CPU 解析增强 + `dumpsys cpuinfo` 兜底 |
| `src/views/device-space/PerfMonitorPage.vue` | 重写 | 移除 gauges/alert/log 按钮；PSS 改 div click；CSV 改 Tauri dialog；内存 legend 移上方；集成自动日志到生命周期 |
| `src/stores/usePerfMonitorStore.ts` | 修改 | 移除 `alertEnabled`/`thresholdCpu`/`thresholdMem`/`lastAlertTime` 及告警逻辑 |
| `src/composables/useI18n.ts` | 修改 | 移除告警/阈值/追踪相关翻译键 |
| `DEVELOPMENT.md` | 修改 | 新增 Phase 44 说明 |
...
## Phase 45 — PerfMonitor 性能与稳定性修复（已完成 ✅）

> 修复用户反馈的 5 个问题：应用内存折线图空白、tooltip 纵坐标 label 干扰、内存单位不统一、设备跑崩、logs 目录为空。

### 问题修复清单

| # | 问题 | 修复方式 |
|---|------|----------|
| 1 | 应用内存折线图（Top N / 单个应用）都是空的 | Rust `adb.rs` 中 `dumpsys meminfo` fallback 解析逻辑错误，条件 `pss_line.chars().all(|c| c.is_ascii_digit() \|\| c.is_whitespace())` 永远不会命中（原字符串末尾是 `)` 不是 `:`/`K`/`kB`）。重写为按 whitespace token 提取首数字作为 PSS，中间 token 过滤后拼接为进程名，支持 `"12345K: com.app (pid 1234)"` 和 `"12345 kB: com.app (pid 1234)"` 两种格式 |
| 2 | 鼠标悬停时左侧显示纵坐标值 | 所有图表（memChart / topAppChart / singleAppChart）tooltip 中 `axisPointer.label` 显式设为 `show: false`，避免 crosshair 在 y 轴左侧弹出数值 label |
| 3 | 内存使用趋势单位不统一 | 新增 `formatMem(v)` 统一输出 `xxGB(XXMB)` 格式；memChart tooltip `valueFormatter` 改用 `formatMem`；yAxis 简化为 `2.5G` / `2560M` 避免重叠 |
| 4 | 设备跑崩 / 前端卡死 | ① `collectLogs` 由 `Promise.allSettled` 8 命令并行改为**串行执行**，减轻设备 ADB 通道瞬时压力；② `logcat` 限制 `-t 5000` 行防止缓冲区无限增长；③ `dumpsys batterystats` 换为轻量 `dumpsys battery`；④ `top -n 1 -b` 换为不会挂起的 `ps -A`；⑤ 日志采集间隔从 15s 延长至 **30s**；⑥ `usePerfMonitorStore.addPoint()` 中新增**进程 Map 清理**：只保留当前 snapshot 中出现的进程 + `singleAppName`，防止 `processPssHistory` 无限膨胀导致前端排序卡死 |
| 5 | logs 文件夹为空 | `collectLogs` 中 `writeTextFile` 改为 `await` 等待完成后再 `push` 路径；`stopPolling` 最终日志捕获改为 `async` 并确保 `await writeTextFile` 完成；移除未使用的 `fileNames` 死代码 |

### 文件变更

| 文件 | 修改类型 | 变更说明 |
|------|----------|----------|
| `src-tauri/src/adb.rs` | 修改 | `PerfProcess` 新增 `Clone`；`dumpsys meminfo` fallback 解析逻辑重写 |
| `src/stores/usePerfMonitorStore.ts` | 修改 | `addPoint()` 末尾增加进程 Map 清理逻辑，防止内存泄漏 |
| `src/views/device-space/PerfMonitorPage.vue` | 修改 | `formatMem()` 新增；图表 tooltip/yAxis formatter 调整；`collectLogs` 串行化+命令轻量+限制行数；`stopPolling` 改为 async；日志间隔改为 30s |
...
## Phase 46 — PerfMonitor 实时日志与交互修复（已完成 ✅）

> 基于用户反馈的 6 项改进：启动选目录 + 实时 logcat 流式抓取、Top 应用重复、单个应用无数据、CSV 单位统一、导出报告精简。

### 改动清单

| # | 需求 | 修复方式 |
|---|------|----------|
| 1 | 启动时弹窗选目录，log 实时输出到电脑 | ① `startPolling` 改为先 `open({ directory: true })` 让用户选目录；② Rust 端新增 `LogcatState` + `adb_logcat_start` / `adb_logcat_stop` 命令，spawn `adb logcat` 进程并将 stdout 实时写入本地 `logcat.txt`；③ 采集开始时抓 `versionInfo.txt`、`anr/`、`tombstone/` 及 ps/df/battery/proc_stat 等一次性日志；④ `collectLogs` 简化为只每 30s 抓 `meminfo/`；⑤ `stopPolling` 停止 logcat 流并抓最终 meminfo/anr/tombstone |
| 2 | perf_top_apps.csv 缺少单位 | 列头改为 `"应用名(KB)"` |
| 3 | perf_data.csv 单位缺少括号 | 列头统一为 `CPU(%)、MemUsed(KB)、MemTotal(KB)` 等带括号格式 |
| 4 | Top 应用图表有很多重复 | `updateTopAppChart` 的 `setOption` 增加 `{ replaceMerge: ['series'] }`，echarts 默认增量更新导致旧 series 残留，现在强制替换 |
| 5 | 单个应用内存没有数据 | ① `appList` 去掉 `!p.name.includes(':')` 过滤，让 service 进程也能被选择；② `detectForegroundApp` 不再直接设置检测到的包名，而是先在 `snapshot.procs` 中找精确匹配或 `startsWith(pkg + ':')` 的进程名，确保 `singleAppName` 和 `processPssHistory` key 一致 |
| 6 | 导出报告只保留 html + csv | 去掉 `exportReport` 中 `logs/` 目录复制逻辑及 `logFileCount` 统计，报告仅包含 3 个 csv + 1 个 html |

### 文件变更

| 文件 | 修改类型 | 变更说明 |
|------|----------|----------|
| `src-tauri/src/lib.rs` | 修改 | 新增 `LogcatState`、`adb_logcat_start`、`adb_logcat_stop`；导入 `BufRead`、`Write` trait |
| `src/views/device-space/PerfMonitorPage.vue` | 修改 | `open`/`invoke`/`pullFile` 导入；`startPolling`/`stopPolling`/`collectLogs` 重写；`captureOneTimeLogs` 新增；`appList` 过滤条件放宽；`detectForegroundApp` 匹配实际进程名；`updateTopAppChart` `replaceMerge`；CSV 列头加括号；导出报告去掉 logs |
...
## Phase 47 — PerfMonitor 解析修复 + 导出图片 + 异常处理（已完成 ✅）

> 基于用户反馈的 6 项修复：① CPU 使用率趋势无数据；② Top20 应用新进前 N 时未移除最后一个；③ 单个应用内存（`com.whaletv.launcher`）无数据；④ 暂停后导出报告缺少 loading 动效；⑤ 四个板块折线图未导出 PNG；⑥ 异常情况（ADB 断连、无数据导出、间隔变更）补全。

### 改动清单

| # | 需求 | 修复方式 |
|---|------|----------|
| 1 | CPU 使用率趋势无数据 | ① 实测 `top -n 1 -b` 输出 `400%cpu 103%user 0%nice 19%sys 274%idle 0%iow 3%irq 0%sirq 0%host`，旧代码反向查找第一个可解析浮点数错误地取到 `19%sys`（19.0）；② 重写：解析 `400%cpu` 为 `total_capacity`、`274%idle` 为 `idle_val`，CPU 总使用率 = `(total_capacity - idle_val) / total_capacity * 100`（如 `(400-274)/400*100 = 31.5%`）；③ 单进程 CPU 改用固定列 `parts[8]`（%CPU），旧代码反向迭代误取 `parts[9]`（%MEM） |
| 2 | Top20 新进应用未挤掉最后一个 | `updateTopAppChart` / `updateCpuChart` 完全重构：每次先按 `lastCpu`/`lastPss` 排序后取前 N，再用 `setOption({ series }, { replaceMerge: ['series'] })` 完整替换 series 数组，echarts 自动移除被挤掉的旧应用 series |
| 3 | 单个应用内存无数据（`com.whaletv.launcher`） | ① 发现 `ps -A -o PID,PSS,NAME` 在目标设备返回帮助文本（不支持 `-o` 字段选择），改用默认列的 `ps -A`；② PSS 主数据源改为 `dumpsys meminfo`，发现实际输出 `    400,056K: com.whaletv.launcher (pid 1428 / activities)` 含千分位逗号，新增 `tokens[0].replace(',', "")` 清洗；③ `ps -A` 的 RSS 作为 dumpsys 未覆盖进程的回退 PSS |
| 4 | 暂停后导出缺少 loading 动效 | 新增全屏遮罩：`isExporting` 状态 + `<div v-if="isExporting" class="fixed inset-0 z-[9998] ...">`，导出过程中显示旋转 spinner 与"正在导出报告..."提示；`exportReport` 和 `autoExportReport` 均置 `try { isExporting.value = true } finally { isExporting.value = false }` |
| 5 | 四个折线图未导出 PNG | 新增 `base64ToBytes` 工具函数；导出时调用 `chart.getDataURL({ pixelRatio: 2, backgroundColor: '#fff', type: 'png' })` 提取 base64 后用 `writeFile`（非已废弃的 `writeBinaryFile`）写入 `chart_cpu.png`/`chart_memory.png`/`chart_top_apps.png`/`chart_single_app.png`；HTML 报告模板新增 `<img class="chart">` 引用 |
| 6 | 异常情况补全 | ① **ADB 断连**：`pollOnce` 累计 `pollErrorCount`，连续 3 次失败 toast 警告，连续 10 次自动 `stopPolling`；成功采集时计数清零；② **无数据导出**：`stopPolling` 自动导出前检查 `store.history.length === 0`，为空则跳过并提示"未采集到数据，跳过自动导出"；③ **间隔变更**：`selectInterval` 检测 `store.isCollecting && pollTimer` 时重启 `setInterval(pollOnce, v)` |

### 文件变更

| 文件 | 修改类型 | 变更说明 |
|------|----------|----------|
| `src-tauri/src/adb.rs` | 修改 | `get_perf_snapshot` 重写：① shell 命令从 `ps -A -o PID,PSS,NAME` 改为 `ps -A`；② CPU total 解析 `400%cpu`/`274%idle` 计算 `(capacity-idle)/capacity*100`；③ 单进程 CPU 用固定列 `parts[8]`；④ dumpsys meminfo PSS `tokens[0].replace(',', "")` 去逗号；⑤ 用 HashMap 合并 PID→PSS 和 PID→CPU；⑥ `procs.truncate(50)` |
| `src/views/device-space/PerfMonitorPage.vue` | 修改 | ① `writeBinaryFile`→`writeFile`（4 处 PNG 导出）；② `updateCpuChart`/`updateTopAppChart` 完全重建 series + `replaceMerge: ['series']`；③ 新增 `isExporting` 遮罩 + spinner；④ `exportReport`/`autoExportReport` 导出 4 个 PNG 并在 HTML 模板引用；⑤ `pollOnce` 新增 `pollErrorCount`（3 警告/10 停止）；⑥ `selectInterval` 收集中重启 timer；⑦ `stopPolling` 无数据跳过自动导出；⑧ 新增 `base64ToBytes` 工具函数 |
| `src-tauri/capabilities/default.json` | 修改 | 移除无效权限 `fs:allow-write-binary-file`（plugin-fs 已无此权限，二进制写入由 `fs:allow-write-file` 覆盖） |

### 关键技术点

- **Android `top` 输出格式**：`400%cpu 103%user 0%nice 19%sys 274%idle 0%iow 3%irq 0%sirq 0%host`，需用 `%cpu` 容量减去 `%idle` 计算实际使用率，而非直接取 `%sys`
- **Android `top` 单进程列**：`0=PID, 1=USER, 2=PR, 3=NI, 4=VIRT, 5=RES, 6=SHR, 7=S, 8=%CPU, 9=%MEM, 10=TIME+, 11+=ARGS`
- **`ps -A -o` 兼容性**：许多 Android 设备的 toybox `ps` 不支持 `-o` 字段选择，应使用默认列 `ps -A`
- **`dumpsys meminfo` PSS 千分位**：`400,056K:` 需 `replace(',', "")` 后再 parse
- **ECharts `replaceMerge`**：用于动态 series 数组的完整替换，自动移除不在新数组中的旧 series，适合 Top N 动态进出场景
- **Tauri plugin-fs 二进制写入**：使用 `writeFile(path, Uint8Array)`，对应权限 `fs:allow-write-file`（`writeBinaryFile` 与 `fs:allow-write-binary-file` 均已废弃）
...
## Phase 48 — PerfMonitor 日志合并 + 图例交互 + 交互式 HTML 报告（已完成 ✅）

> 基于用户反馈的 4 项改进：① log 目录不再区分 final/non-final；② PNG 图片包名显示不全；③ Top N CPU/内存图表鼠标悬停弹窗超出屏幕；④ HTML 报告增加可交互的图表（点击图例可切换应用显示）。

### 改动清单

| # | 需求 | 修复方式 |
|---|------|----------|
| 1 | log 不分 final/非 final，汇总一个目录 | `captureOneTimeLogs` 移除 `isFinal` 参数与 `suffix` 变量；`anr/`、`tombstone/` 目录不再带 `_final` 后缀，停止时再次抓取会覆盖/累积到同一目录；`stopPolling` 调用处同步移除 `true` 参数 |
| 2 | PNG 图片包名显示不全 | 新增 `exportChartPNG(chart, filePath, hasMultipleSeries)` 工具：多 series 图表导出时临时切换 `legend: { show: true, orient: 'vertical', right: 0, top: 'middle', type: 'scroll' }` 并设置 `textStyle: { width: 220, overflow: 'break' }` 在右侧竖排显示完整包名；导出后再恢复原 legend（隐藏）配置；CPU/TopApp 多 series 图表导出 PNG 时还会临时清空 `cpuHiddenApps`/`topAppHiddenApps` 让所有 Top N 应用都出现在 PNG 中，导出后再恢复用户筛选状态 |
| 3 | Top N CPU/内存图表鼠标悬停弹窗超出屏幕 | ① CPU/TopApp 图表移除 echarts 默认底部 legend（`legend: { show: false }`），改为标题旁自定义「应用筛选」下拉按钮（`filter_list` 图标 + N/total 计数徽标）；② 下拉面板支持「全选/全不选」与单个勾选；③ 状态用 `cpuHiddenApps`/`topAppHiddenApps`（`Set<string>`）维护被隐藏的应用；④ `updateCpuChart`/`updateTopAppChart` 在排好序后增加 `.filter(p => !hidden.has(p.name))`；⑤ tooltip 增加 `confine: true, appendTo: 'body'` 防止弹出超出屏幕；⑥ 新应用进入 Top N 时默认勾选（visible），仅用户手动取消过的应用保持隐藏 |
| 4 | 报告新增交互式 HTML 图表 | 新增 `buildInteractiveChartsHtml(pts, topMem, topCpu, singleName, singleData)`：将 4 个图表数据序列化为 JSON 嵌入 `<script type="application/json">`，内联 JS 从 jsdelivr CDN 加载 echarts@5（失败自动回退到 bootcdn，仍失败时显示友好提示），渲染 4 个可点击图例切换的交互式 ECharts 图表；HTML 模板里在「折线图 PNG」和「导出文件清单」之间插入 `${interactiveHtml}`；新增 `.interactive-chart` CSS；导出文件清单更新说明 perf_report.html 含交互式图表（需联网加载图表库） |

### 文件变更

| 文件 | 修改类型 | 变更说明 |
|------|----------|----------|
| `src/views/device-space/PerfMonitorPage.vue` | 修改 | ① `captureOneTimeLogs` 去掉 `isFinal` 参数与 `suffix`，anr/tombstone 汇总到单一目录；② 新增 `cpuFilterOpen`/`topAppFilterOpen`/`cpuHiddenApps`/`topAppHiddenApps`/`cpuAppNames`/`topAppNames`/`toggleCpuApp`/`toggleTopApp`/`cpuSelectAll`/`cpuSelectNone`/`topAppSelectAll`/`topAppSelectNone` 等 12 个状态/方法；③ CPU/TopApp 标题旁新增「应用筛选」下拉按钮 + Teleport 面板；④ `handleDropdownClick` 新增筛选面板点击外部关闭；⑤ `initCharts` 中 CPU/TopApp 改 `legend: { show: false }`，tooltip 加 `confine/appendTo`；⑥ `updateCpuChart`/`updateTopAppChart` 加 `.filter(p => !hidden.has(p.name))`；⑦ `clearHistory` 重置 hiddenApps；⑧ `watch topAppCount` 同时更新 CPU 图表；⑨ 新增 `exportChartPNG` 工具函数；⑩ `exportReport`/`autoExportReport` 用 `exportChartPNG` 替代手动 `getDataURL`+`writeFile`，导出前临时清空筛选；⑪ 新增 `buildInteractiveChartsHtml` 函数；⑫ 两处 HTML 模板插入 `${interactiveHtml}` 与 `.interactive-chart` CSS |

### 关键技术点

- **Vue SFC 中嵌入 `</script>` 字符串**：HTML5 解析规则会让 Vue 解析器把 JS 模板字符串里的 `</script>` 误判为 `<script setup>` 块的结束标签，导致 "Unterminated template literal"。修复方式：在源码模板字符串里写 `<\/script>`（`\/` 在 JS 字符串里就是 `/`，不影响运行结果，但能避免 Vue 解析器误判）
- **ECharts 临时切换 legend 导出 PNG**：导出前 `setOption` 切到右侧竖排 legend + 宽 grid，`setTimeout(30ms)` 等待重渲染，`getDataURL` 后再 `setOption` 恢复原配置；CPU/TopApp 多 series 图表同时清空 hiddenApps 让所有应用入图，导出后再恢复用户筛选
- **自定义图例 vs 默认 legend**：默认 `legend: { bottom: 0, type: 'scroll' }` 在多应用场景下会挤压图表底部空间且 tooltip 弹出会与 legend 文本重叠超出屏幕；改为标题旁下拉面板后图表底部可全用作 dataZoom，悬停 tooltip 用 `confine: true` 限制在容器内
- **交互式 HTML 报告离线降级**：CDN 加载失败时显示友好提示，并指引用户查看上方的静态 PNG 图表；JSON 数据通过 `<script type="application/json">` 嵌入避免 XSS 风险，并 `replace(/</g, '\\u003c')` 转义防止 `</script>` 注入

**编译验证**：
| 检查项 | 结果 |
|--------|------|
| `npm run build` (vue-tsc + vite) | ✅ 通过 |
| `cargo check` | 未涉及 Rust 改动 |

## Phase 49 — PerfMonitor Top N 「顶出最后一个」修复（已完成 ✅）

> 修复用户反馈：CPU 使用率和 Top N 应用内存两块图表，当有新的更高占用的应用进入时，没有把最低的一个顶掉。经排查发现两处 Bug：

### 改动清单

| # | 问题根因 | 修复方式 |
|---|----------|----------|
| 1 | Rust `get_perf_snapshot` 仅按 PSS 降序排序并 `truncate(50)`，高 CPU 但低 PSS 的进程在到达前端之前就被丢弃，导致 CPU 图表永远看不到这些高 CPU 应用，自然无法「顶出」最低的那个 | 改为「按 PSS 降序保留前 40 + 按 CPU 降序补齐未包含的高 CPU 进程，总共最多 50」的混合策略：先 `sort_by(pss desc)` 后 `drain(..40)` 收集到 `kept`；再以 `HashSet<String>` 记录已保留进程名；剩余 `procs` 按 `cpu_percent desc` 排序后逐个补入 `kept`，直到达到 50 个上限或耗尽 |
| 2 | 前端 `updateCpuChart`/`updateTopAppChart` 中 `.filter(p => !hidden.has(p.name))` 在 `.slice(0, topAppCount)` 之后执行，导致被用户隐藏的应用仍占用 Top N 名额（例如 Top 20 隐藏 5 个，实际只显示 15 个） | 调换顺序：先 `.sort()` 再 `.filter()` 最后 `.slice(0, topAppCount)`，确保「Top N 个可见应用」始终是当前活跃应用中值最高的 N 个，新进入的高占用应用会自然顶出最低的可见应用 |

### 文件变更

| 文件 | 修改类型 | 变更说明 |
|------|----------|----------|
| `src-tauri/src/adb.rs` | 修改 | `get_perf_snapshot` 末尾的 `procs.sort_by(pss desc); procs.truncate(50)` 重写为「PSS 前 40 + CPU 补齐到 50」混合保留策略；新增 `kept`/`kept_names` 局部变量；`kept_names` 使用 `HashSet<String>`（owned）避免与 `kept.push()` 的可变借用冲突 |
| `src/views/device-space/PerfMonitorPage.vue` | 修改 | `updateCpuChart` 与 `updateTopAppChart` 中将 `.filter(p => !hidden.has(p.name))` 从 `.slice()` 之后移到 `.slice()` 之前；更新注释说明为何要先 filter 后 slice |

### 关键技术点

- **混合保留策略**：单纯按 PSS 排序截断会丢失 CPU 视角的关键进程；单纯按 CPU 排序会丢失内存视角的关键进程。「按 PSS 取前 40 + 按 CPU 补齐到 50」保证两个图表都能拿到足够候选应用，前端 `processPssHistory`/`processCpuHistory` Map 中始终有充足候选，Top N 排序选优才有意义
- **filter-before-slice 不变量**：用户隐藏的应用应当「从候选池中排除」而非「占用名额后不显示」。顺序必须是 sort → filter → slice，否则隐藏 5 个应用后图表只剩 15 条线，违反 Top N 契约
- **Rust 借用冲突规避**：`kept.iter().map(...).collect::<HashSet<&str>>()` 的不可变借用会贯穿整个循环直到 `kept.push()` 时引发 E0502；改用 `HashSet<String>`（owned）让不可变借用立即释放，循环内可自由 `push`
...
## Phase 50 — PerfMonitor 七项修复（已完成 ✅）

> 用户反馈 7 个问题：① CPU 超过 100% 不合理；② 加载历史只加载内存趋势其余三个空；③ HTML 报告不再需要静态折线图和 4 张 PNG；④ 交互式图表图例放侧边字体加大；⑤ 隔天是否显示日期；⑥ CPU 下拉框无法点击且被 TopApp 同步；⑦ 内存使用趋势改为设备内存趋势。

### 改动清单

| # | 问题 | 修复方式 |
|---|------|----------|
| 1 | CPU 超 100% | `adb.rs` 从 `%cpu` 行推导 `core_count = total_capacity / 100`（如 `400%cpu` → 4 核），每个进程 `cpu_percent / core_count` 后 clamp 到 `[0, 100]`；fallback（dumpsys cpuinfo）也加 `.clamp(0, 100)` |
| 2 | 加载历史只有内存趋势 | Store 新增 `restoreFullSession(data)` 方法恢复 `history` + `processPssHistory` + `processCpuHistory` + `singleAppName` + `topAppCount` + `cpuTopNCount`；`doSaveSession` 序列化为对象（含所有 Map 的 `entries()`）；`doLoadSession` 兼容旧格式（纯数组）和新格式（对象），加载后调用全部 4 个 update 函数 |
| 3 | 删除 HTML 静态折线图 + 4 张 PNG | `exportReport`/`autoExportReport` 移除 `<div class="card"><h2>折线图</h2>...4 img...</div>`、移除文件清单中 4 条 PNG 条目、移除 `exportChartPNG` 4 次调用及临时清空/恢复 hiddenApps 逻辑；删除 `exportChartPNG` 和 `base64ToBytes` 函数；移除 `writeFile` import |
| 4 | 交互式图表图例侧边 + 字体加大 | `buildInteractiveChartsHtml` 的 `opts.legend` 从 `bottom: 0, textStyle.fontSize: 10` 改为 `orient: 'vertical', right: 0, top: 'middle', itemWidth: 14, textStyle: { fontSize: 13, width: 200, overflow: 'truncate' }`；`grid.right` 从 30 改为 220；图表高度从 300px 增加到 360px |
| 5 | 隔天显示日期 | 应用内 4 个图表 `xAxis.axisLabel` 增加 `formatter`：检查 `store.history` 首/末时间戳是否同一天，不同天显示 `MM-DD HH:MM`，同天显示 `HH:MM:SS`；交互式 HTML 图表 `xAxis.axisLabel.formatter` 同理，从嵌入 JSON 的所有 series 时间戳中取 min/max 判断是否跨天 |
| 6 | CPU 下拉框无法点击 + 被 TopApp 同步 | ① Store 新增 `cpuTopNCount` 独立于 `topAppCount`；② `selectCpuTopN` 改为 `store.cpuTopNCount = n`；③ `updateCpuChart`/`cpuAppNames` 改用 `store.cpuTopNCount`；④ 模板 CPU 下拉按钮 `div` 加 `ref="cpuDropdownRef"`（之前无 ref 导致 `handleDropdownClick` 总是关闭下拉）；⑤ `handleDropdownClick` 改为 `if (cpuDropdownOpen && cpuDropdownRef && !cpuDropdownRef.contains(t))` 而非无条件关闭；⑥ `toggleCpuDropdown` 改用 `cpuDropdownRef.value.getBoundingClientRect()` 定位；⑦ watch 拆为 `topAppCount`→`updateTopAppChart` 和 `cpuTopNCount`→`updateCpuChart` 两条 |
| 7 | 内存使用趋势→设备内存趋势 | `useI18n.ts` 中 `perf.memTrend` 从「内存使用趋势」改为「设备内存趋势」（en: "Device Memory Trend"）；交互式 HTML 图表标题同步改为「设备内存趋势」 |

### 文件变更

| 文件 | 修改类型 | 变更说明 |
|------|----------|----------|
| `src-tauri/src/adb.rs` | 修改 | 新增 `core_count` 变量从 `%cpu` 行推导；per-process `cpu_val / core_count` 后 clamp `[0, 100]`；fallback cpu_total 加 `.clamp(0, 100)` |
| `src/stores/usePerfMonitorStore.ts` | 修改 | 新增 `cpuTopNCount = ref(20)`；新增 `restoreFullSession(data)` 恢复全部状态；`clearHistory` 额外清空 `singleAppName`；return 导出 `cpuTopNCount` 和 `restoreFullSession` |
| `src/composables/useI18n.ts` | 修改 | `perf.memTrend` zh: "设备内存趋势"、en: "Device Memory Trend" |
| `src/views/device-space/PerfMonitorPage.vue` | 修改 | ① CPU 下拉按钮加 `ref="cpuDropdownRef"`，绑定/显示 `store.cpuTopNCount`；② `selectCpuTopN`/`cpuAppNames`/`updateCpuChart` 改用 `cpuTopNCount`；③ `handleDropdownClick` CPU 分支改为带 ref 检查；④ `doSaveSession` 序列化全部状态为对象；⑤ `doLoadSession` 兼容新旧格式 + 调用 4 个 update 函数；⑥ `exportReport`/`autoExportReport` 删除静态折线图 div、PNG 文件清单条目、`exportChartPNG` 调用及临时 hiddenApps 逻辑；⑦ 删除 `exportChartPNG`/`base64ToBytes` 函数、移除 `writeFile` import；⑧ `buildInteractiveChartsHtml` legend 改右侧竖排 + 字体 13px + grid.right 220 + 图高 360px；⑨ 应用内 4 图表 xAxis 加 time formatter（跨天显示日期）；⑩ 交互式 HTML xAxis 加 formatter；⑪ `initCharts` watch 拆分；⑫ 导出报告中 topCpu slice 用 `cpuTopNCount` |

### 关键技术点

- **per-process CPU 归一化**：Android `top` 的 `%CPU` 列是相对于单个核心的百分比（8 核设备最高 800%），而非设备总 CPU 容量。从 `%cpu` 行的 total_capacity 推导出核数后除以核数，得到「占设备总 CPU 的百分比」，范围 `[0, 100]`
- **CPU 下拉框点击失效根因**：`handleDropdownClick` 中 `if (cpuDropdownOpen.value) { cpuDropdownOpen.value = false }` 无条件关闭，因为按钮没有 ref 所以无法判断点击是否在按钮内。其他三个下拉框（interval/topN/filter）都有 ref 并检查 `!ref.contains(t)`，唯独 CPU 下拉框漏了
- **保存/恢复全量状态**：旧版 `restoreHistory` 只恢复 `history` 数组，`processPssHistory`/`processCpuHistory`/`singleAppName` 均丢失。新版 `restoreFullSession` 通过 `Array.from(map.entries())` 序列化 Map，`new Map(entries)` 反序列化，兼容旧版纯数组格式
- **跨天时间格式化**：ECharts `type: 'time'` 轴默认只显示时间不显示日期。自定义 formatter 在渲染每个 tick 时检查数据首末时间戳是否同一天：同天显示 `HH:MM:SS`，跨天显示 `MM-DD HH:MM`，无需修改 ECharts 内部逻辑
...
## Phase 51 — PerfMonitor 长周期运行与非阻塞优化（已完成 ✅）

> 用户反馈 3 个问题：① `perf.sessionDeleted` 缺少翻译；② 性能监控切页面时不阻塞主线程、不停止采集；③ 支持长周期压测（3 天~1 个月），保证正常运行和生成报告。

### 改动清单

| # | 问题 | 修复方式 |
|---|------|----------|
| 1 | `perf.sessionDeleted` 缺翻译 | `useI18n.ts` 新增 `perf.sessionDeleted` zh: "会话已删除" / en: "Session deleted" |
| 2 | 切页面阻塞 + 可能停止 | ① 新增 `isPageActive` ref 跟踪页面激活状态；② `onDeactivated` 设 `isPageActive=false`、`onActivated` 设 `true` 并重绘全部图表；③ `pollOnce` 中仅当 `isPageActive.value` 为 true 时才调用 `updateCharts`/`updateTopAppChart`/`updateSingleAppChart`；④ keep-alive 保证定时器不停止、数据采集持续 |
| 3 | 长周期运行支持 | 四项子修复见下 |
...
| 子项 | 问题 | 修复方式 |
|------|------|----------|
| 3a | 内存溢出风险 | Store `maxPoints` 从 300 增到 86400（24h@1s / 48h@2s / 5d@5s / 10d@10s / 30d@30s）；50 进程 × 86400 点 × 32 字节 ≈ 138MB 最坏情况 |
| 3b | 数据丢失风险 | 新增 Rust `append_text_file` 命令（`OpenOptions::create+append`）；采集期间每 30 个数据点流式追加到 `perf_data.csv`；`stopPolling` 时 flush 剩余 buffer；`autoExportReport` 不再重写 `perf_data.csv`（已存在） |
| 3c | `Math.max(...arr)` 栈溢出 | 新增 `safeMax(arr)` 函数用循环遍历替代展开调用，避免超过 ~65K 元素时 V8 栈溢出；`exportReport`/`autoExportReport` 中所有 `Math.max(...xxx)` 替换为 `safeMax(xxx)` |
| 3d | HTML 报告过大 | 新增 `downsample<T>(arr, maxLen)` 函数，超过 maxLen 时按 `ceil(len/maxLen)` 步长采样，保留首末点；`exportReport`/`autoExportReport` 生成交互式 HTML 前对所有 series 下采样到 2000 点上限 |

### 文件变更

| 文件 | 修改类型 | 变更说明 |
|------|----------|----------|
| `src-tauri/src/lib.rs` | 新增 | `append_text_file` Tauri 命令：`OpenOptions::new().create(true).append(true).open().write_all()`；注册到 `generate_handler!` 列表 |
| `src/composables/useI18n.ts` | 修改 | 新增 `perf.sessionDeleted` zh/en 翻译 |
| `src/stores/usePerfMonitorStore.ts` | 修改 | `maxPoints` 从 300 改为 86400；注释说明各间隔对应时长 |
| `src/views/device-space/PerfMonitorPage.vue` | 修改 | ① 新增 `isPageActive` ref + `onDeactivated` import；② `pollOnce` 加 `isPageActive` 检查跳过图表渲染；③ `onActivated`/`onDeactivated` 切换 `isPageActive` 并在激活时重绘；④ 新增 `safeMax`/`downsample` 工具函数；⑤ 新增 `csvBuffer`/`CSV_FLUSH_THRESHOLD`/`initCsvFiles`/`bufferCsvPoint`/`flushCsvBuffer` 流式 CSV 写入；⑥ `startPolling` 初始化 CSV 文件头；⑦ `stopPolling` flush 剩余 buffer；⑧ `exportReport`/`autoExportReport` 所有 `Math.max(...)` 替换为 `safeMax()`，交互式 HTML 生成前下采样到 2000 点；⑨ `autoExportReport` 不再重写 `perf_data.csv`（已流式写入） |

### 关键技术点

- **keep-alive + onDeactivated**：PerfMonitorPage 在 keep-alive 的 include 列表中，切换页面时组件不销毁，`onDeactivated` 触发而非 `onUnmounted`。定时器持续运行，ADB 采集不断，仅跳过 ECharts `setOption` 渲染（最大的主线程开销）。返回页面时 `onActivated` 一次性重绘全部图表
- **流式 CSV 追加**：`append_text_file` 使用 Rust `OpenOptions::append(true)`，每次只写入增量数据（~30 行），不读取已有文件，O(1) 复杂度。即使 app 崩溃，最多丢失 30 个数据点（~1 分钟）
- **V8 栈溢出**：`Math.max(...arr)` 使用展开参数（spread），V8 将参数压入调用栈，超过 ~65K 元素会 RangeError。`safeMax` 用 `for...of` 循环遍历，无栈深度限制
- **下采样**：`downsample` 按 `ceil(len/maxLen)` 步长每隔 N 个点取一个，保留首末点确保时间范围完整。2000 点上限保证 HTML 报告 JSON payload ≤ ~200KB per series，浏览器可流畅渲染
...
## Phase 52 — PerfMonitor xAxis/tooltip/loading/标题修复（已完成 ✅）

> 用户反馈 4 个问题：① 横坐标显示 NaN 且重叠；② 暂停时需立即显示 loading 直到保存完成，并有超时保护；③ Top N 应用内存 tooltip 超出窗口；④ CPU 使用率趋势改为「Top N 应用CPU使用率」。

### 改动清单

| # | 问题 | 修复方式 |
|---|------|----------|
| 1 | 横坐标 NaN + 重叠 | ① 根因：ECharts `axisLabel.formatter` 签名是 `(value, index)` 而非 `(params)`，原代码 `params.value` 永远是 `undefined` → `new Date(undefined)` → Invalid Date → `getMonth()` 返回 NaN；② 修复：将 `(params: any) => new Date(params.value)` 改为 `(value: any) => new Date(value)`；③ 加 `isNaN(d.getTime())` 防御；④ 加 `hideOverlap: true` 自动隐藏重叠的刻度标签；⑤ 应用内 4 图表 + 交互式 HTML 图表均同步修复 |
| 2 | 暂停无 loading + 无超时 | ① `stopPolling` 在任何 async 操作之前立即 `isExporting.value = true`；② 60 秒超时定时器：超时后强制 `isExporting = false` + Toast 提示「导出超时」；③ `try/catch/finally` 包裹，`finally` 中 `clearTimeout` + `isExporting = false`；④ Loading 文本改为「正在停止并保存数据...」；⑤ 无数据时跳过 loading 直接 Toast 提示 |
| 3 | Top N tooltip 超出窗口 | CPU 和 TopApp 两个多 series 图表的 tooltip 改为自定义 `formatter`：按值降序排序后只显示 top 5，剩余显示「... 还有 N 个应用」；tooltip 容器加 `max-height:60vh;overflow-y:auto` 保证可滚动；`confine: true` + `appendTo: 'body'` 确保不超出视口 |
| 4 | 标题统一 | 应用内 CPU 图表标题从 `t('perf.cpuTrend')` 改为 `Top{{ store.cpuTopNCount }} 应用CPU使用率`（与 TopApp 内存「Top{{ store.topAppCount }} 应用内存」一致）；交互式 HTML 图表标题从「CPU 使用率趋势 (Top N)」改为「Top N 应用 CPU 使用率」 |

### 文件变更

| 文件 | 修改类型 | 变更说明 |
|------|----------|----------|
| `src/views/device-space/PerfMonitorPage.vue` | 修改 | ① `timeAxisLabel.formatter` 参数从 `(params)` 改为 `(value)`，加 `isNaN` 防御 + `hideOverlap: true`；② 交互式 HTML xAxis formatter 同步修复；③ CPU/TopApp tooltip 改为自定义 formatter（top 5 + max-height + 可滚动）；④ `stopPolling` 重写：立即 `isExporting=true` + 60s 超时 + try/catch/finally；⑤ Loading 文本改「正在停止并保存数据...」；⑥ CPU 图表标题改 `Top{{ store.cpuTopNCount }} 应用CPU使用率`；⑦ 交互式 HTML CPU 标题改「Top N 应用 CPU 使用率」 |

### 关键技术点

- **ECharts axisLabel formatter 签名**：`axisLabel.formatter` 可以是字符串模板或函数。作为函数时签名是 `(value: string|number, index: number) => string`，其中 `value` 对 time 轴是时间戳（number）。误用 `params.value` 会导致 `undefined` → Invalid Date → NaN 显示
- **hideOverlap**：ECharts 5+ 的 axisLabel 属性，自动计算刻度标签宽度并隐藏会重叠的标签，比手动控制 `interval` 更智能
- **tooltip confine + 自定义 formatter**：`confine: true` 将 tooltip 限制在图表容器内，`appendTo: 'body'` 将 DOM 挂载到 body 避免被父容器 `overflow:hidden` 裁剪。自定义 formatter 可控制显示条目数，避免 20 个 series 全部列出导致 tooltip 超出视口高度
- **loading 超时保护**：Promise 可能永远不会 resolve/reject（如 Rust 端死锁），`setTimeout` 60s 后强制清除 loading 状态并提示用户，防止 UI 永久卡死
...
## Phase 53 — PerfMonitor HTML 报告表格增强与整体评估（已完成 ✅）

> 用户反馈 HTML 报告需求：① Top20 应用内存/CPU 表格显示应用的最小值、最大值、平均值；② 单个应用内存显示最小值、最大值、平均值；③ 采集信息下方增加「整体评估」部分（异常情况、内存泄露情况、持续上涨情况），并提示「仅供参考」。

### 改动清单

| # | 需求 | 实现方式 |
|---|------|----------|
| 1 | Top N 表格显示 min/max/avg | 内存表头从 `#/包名/最新 PSS` 改为 `#/包名/最小 PSS/最大 PSS/平均 PSS`；CPU 表头从 `#/包名/最新 CPU%` 改为 `#/包名/最小 CPU%/最大 CPU%/平均 CPU%`；每行用 `safeMin/safeMax/safeAvg` 计算该应用所有数据点的统计值 |
| 2 | 单个应用内存显示 min/max/avg | 从「最后 20 条记录表格」改为「最小 PSS/最大 PSS/平均 PSS/采样数」单行汇总表 |
| 3 | 整体评估部分 | 新增 `buildOverallAssessment()` 函数，插入到「采集信息」与「汇总统计」之间 |

### 整体评估逻辑

| 类别 | 检测规则 |
|------|----------|
| 异常情况 | ① CPU 峰值 > 90% 接近满载，> 80% 较高；② 设备可用内存 < 总内存 10% OOM 风险，< 20% 紧张；③ 单应用 CPU 峰值 > 50% 且 > 均值 3 倍，CPU 突发占用 |
| 内存泄露情况 | 应用 PSS 首 10 点均值 vs 末 10 点均值，末 > 首 × 1.5 判定为「疑似内存泄露」，1.3~1.5 归入「持续上涨」 |
| 持续上涨情况 | ① 应用 PSS 末段均值 > 首段 × 1.3（但 < 1.5）；② 单应用 PSS 同规则；③ 应用 CPU 近期均值 > 30% 持续高占用 |

**免责声明**：评估区域顶部显示黄色警告条「⚠️ 仅供参考：以下评估基于采集期间的统计数据和简单趋势检测，不构成确定性诊断。实际表现可能受设备状态、系统调度、测试场景等多种因素影响，请结合具体场景综合判断。」

### 文件变更

| 文件 | 修改类型 | 变更说明 |
|------|----------|----------|
| `src/views/device-space/PerfMonitorPage.vue` | 修改 | ① 新增 `safeMin`/`safeAvg` 工具函数（`safeMax` 已存在）；② 新增 `buildOverallAssessment()` 函数（~100 行，6 条检测规则）；③ `exportReport` HTML：Top N 内存表头加 min/max/avg 列、Top N CPU 表头加 min/max/avg 列、单应用表格改为汇总表、采集信息后插入 `buildOverallAssessment` 调用、CPU 表标题用 `store.cpuTopNCount`；④ `autoExportReport` HTML：同上同步修改，变量名 `topMem` 而非 `topN` |

### 关键技术点

- **趋势检测阈值**：首末段各取 `min(10, floor(n/2))` 点计算均值，避免单点抖动误判。1.5× 作为「泄露」阈值参考 Android Vitals 告警经验，1.3× 作为「上涨」阈值
- **`safeMin`/`safeAvg`**：与已有的 `safeMax` 同样用循环遍历而非 `Math.min(...arr)` / `arr.reduce()/arr.length`，避免长周期采集（数万数据点）时 V8 栈溢出
- **评估区域位置**：插入在「采集信息」与「汇总统计」之间，符合用户「采集信息下方」的要求，且在数据详情之前先呈现结论性评估，符合「先结论后细节」的阅读习惯
...
## Phase 54 — PerfMonitor tooltip 与单个应用内存修复（已完成 ✅）

> 用户反馈 3 个问题：① 鼠标悬停在 CPU 使用率折线图上只显示时间，没有应用数据，Top N 应用内存也有同样问题；② 单个应用内存没有数据了；③ 排查还有哪里被改坏了。

### 改动清单

| # | 需求 | 实现方式 |
|---|------|----------|
| 1 | 修复 CPU/TopApp tooltip 只显示时间 | tooltip formatter 中 `p.value` 在 `data: [[time, value], ...]` 格式下是数组 `[time, value]` 而非数字，`数组 >= 0` 恒为 false 导致所有 series 被过滤。新增 `getVal()` 提取实际数值，filter/sort/format 全部使用提取后的值 |
| 2 | 修复单个应用内存无数据 | Rust `get_perf_snapshot` 增加 `watch_app: Option<&str>` 参数，截断前从 procs 中提取 watch_app 对应进程单独保存，截断后加回 kept，确保用户选择的应用始终保留在返回结果中 |
| 3 | 排查其他回归 | 确认 `pollOnce` 中 `updateCharts()` 内部已调用 `updateCpuChart()`（非 bug）；`stopPolling` 超时/loading 逻辑正确；`exportReport`/`autoExportReport` 中 singleData 获取正确；未发现其他回归 |

### 根因分析

#### 问题 1：tooltip 只显示时间

Phase 52 添加自定义 tooltip formatter 时，未考虑 ECharts time axis 下 `data: [[time, value], ...]` 格式的 `p.value` 是数组。原代码：

```javascript
const sorted = params
  .filter((p) => p.value !== null && p.value !== undefined && p.value >= 0)  // BUG
  .sort((a, b) => (b.value ?? 0) - (a.value ?? 0))  // 数组相减得到 NaN
```

`[1700000000000, 5.5] >= 0` → JavaScript 将数组转字符串再转 NaN → `NaN >= 0` → false，所有 series 被过滤，最终只输出时间行。

#### 问题 2：单个应用内存无数据

Rust `get_perf_snapshot` 的截断逻辑（Phase 49）保留 top 40 PSS + top 10 CPU = 50 个进程。如果用户选择的 singleAppName 对应的进程不在 top 50 中，会被丢弃，导致 store `processPssHistory` 中没有该应用的数据，且剪枝逻辑会删除已有条目。

### 文件变更

| 文件 | 修改类型 | 变更说明 |
|------|----------|----------|
| `src/views/device-space/PerfMonitorPage.vue` | 修改 | ① CPU chart tooltip formatter：新增 `getVal()` 提取 `Array.isArray(p.value) ? p.value[1] : p.value`，filter/sort/format 全用提取值；② TopApp chart tooltip formatter：同上修复；③ `pollOnce` 中 `getSnapshot` 调用传入 `store.singleAppName` 作为 watch_app |
| `src/composables/usePerfMonitor.ts` | 修改 | `getSnapshot` 增加 `watchApp?: string` 参数，通过 `invoke` 传给 Rust |
| `src-tauri/src/adb.rs` | 修改 | `get_perf_snapshot` 签名增加 `watch_app: Option<&str>`；截断前用 exact/prefix/partial 三级匹配从 procs 中提取 watch_app，截断后加回 kept |
| `src-tauri/src/lib.rs` | 修改 | `perf_get_snapshot` 命令增加 `watch_app: Option<String>` 参数，通过 `as_deref()` 传给 adb |

### 关键技术点

- **ECharts tooltip `p.value` 类型**：当 series `data` 是 `[[time, value], ...]`（time axis）时，tooltip formatter 的 `params[i].value` 是数组 `[time, value]` 而非数字。必须先用 `Array.isArray(p.value) ? p.value[1] : p.value` 提取实际值，否则 `数组 >= 0` / `数组.toFixed()` 等运算都会失效
- **watch_app 匹配策略**：与 store `addPoint` 中 singleAppName 匹配逻辑保持一致（exact → prefix `name:` → partial includes），确保用户输入 partial 包名（如 `whaletv`）也能匹配到 `com.whaletv.launcher`
- **截断前提取而非截断后补查**：在 `procs.drain(..pss_keep)` 之前用 `procs.remove(i)` 把 watch_app 移出，避免它占用 top 40 PSS 的名额，截断后再 `kept.push(wp)` 加回，这样既保证 watch_app 保留，又不影响 top 40 PSS 的排序准确性
...
## Phase 55 — PerfMonitor 单个应用内存修复与 tooltip 全量展示（已完成 ✅）

> 用户反馈 2 个问题：① 单个应用内存还是不显示数据（Phase 54 修复后仍未生效）；② 折线图悬停 tooltip 只显示 Top 5 个应用 + "还有 x 个应用"，要完整展示出来，在窗口范围内展示。

### 改动清单

| # | 需求 | 实现方式 |
|---|------|----------|
| 1 | 修复单个应用内存仍无数据 | Phase 54 中 Tauri invoke 参数名用了 `watchApp`（驼峰），但 Rust 端命令参数是 `watch_app`（蛇形），Tauri 不会自动做命名转换，导致 Rust 端永远收到 `None`，watch_app 功能完全失效。统一改为 `watch_app` |
| 2 | tooltip 完整展示所有应用 | 去掉 CPU 和 TopApp 两个图表 tooltip 的 `slice(0, 5)` 截断，改为显示所有 series；保留 `max-height:60vh; overflow-y:auto` 确保在窗口范围内可滚动展示 |

### 根因分析

#### 问题 1：单个应用内存仍无数据

Phase 54 已经在 Rust 端加了 `watch_app: Option<String>` 参数，前端也传了参数，但**参数名不匹配**：

```javascript
// 前端（错误）：传的是 watchApp（驼峰）
invoke("perf_get_snapshot", { serial, watchApp: watchApp ?? null })

// Rust 端：期望的是 watch_app（蛇形）
async fn perf_get_snapshot(serial: String, watch_app: Option<String>)
```

Tauri v2 的命令参数命名是**精确匹配**的，不会自动做 snake_case ↔ camelCase 转换。所以 Rust 端的 `watch_app` 永远收到 `None`，整个 watch_app 保留逻辑形同虚设，用户选择的应用还是会被 top 50 截断逻辑丢掉。

**教训**：Tauri 命令参数名必须前后端完全一致，建议统一用 snake_case（与 Rust 命名风格一致）。

### 文件变更

| 文件 | 修改类型 | 变更说明 |
|------|----------|----------|
| `src/composables/usePerfMonitor.ts` | 修改 | `getSnapshot` 参数名从 `watchApp` 改为 `watch_app`，invoke 传参也改为 `watch_app` |
| `src/views/device-space/PerfMonitorPage.vue` | 修改 | ① CPU chart tooltip：去掉 `slice(0, 5)` 截断和 "还有 x 个应用" 提示，改为显示所有 series；② TopApp chart tooltip：同上 |

**编译验证**：
| 检查项 | 结果 |
|--------|------|
| `npm run build` (vue-tsc + vite) | ✅ 通过 |
| `cargo check` | ✅ 通过（无 Rust 改动） |

## Phase 56 — PerfMonitor 单个应用内存启动后丢失修复（已完成 ✅）

> 用户反馈：单个应用内存还是不显示数据，在启动前输入包名后会显示一下数据，但是点击启动后就没了，抓不到数据。

### 根因分析

`store.clearHistory()` 中把 `singleAppName` 也一起清空了：
...
function clearHistory() {
  ...
  singleAppName.value = ''   // ← BUG：不该清空用户选择的监控目标
}
```

**完整链路**：
1. 用户在输入框填入包名 `com.whaletv.launcher` → `singleAppName = 'com.whaletv.launcher'`
2. 如果之前有历史数据（或刚加载过会话），图表会显示对应数据
3. 用户点击"开始" → `startPolling()` → 第一步就调用 `clearHistory()`
4. `clearHistory()` 把 `singleAppName` 清空为 `''`
5. 后续 `pollOnce` 传 `store.singleAppName` = 空字符串 → Rust 端 `watch_app` 收到 `None` → 不保留目标应用
6. 低内存/CPU 的应用被 top 50 截断丢掉 → 单个应用图表无数据

### 修复方案

从 `clearHistory()` 中移除 `singleAppName.value = ''`。`clearHistory` 的职责是清除采集的数据，不应该清除用户的配置选择（监控目标、Top N 数量等都应该保留）。

### 文件变更

| 文件 | 修改类型 | 变更说明 |
|------|----------|----------|
| `src/stores/usePerfMonitorStore.ts` | 修改 | `clearHistory()` 中移除 `singleAppName.value = ''`，用户选择的监控目标在清除历史后保留 |

**编译验证**：
| 检查项 | 结果 |
|--------|------|
| `npm run build` (vue-tsc + vite) | ✅ 通过 |
| `cargo check` | 无 Rust 改动 |

## Phase 57 — PerfMonitor watch_app 参数名修复与调试日志（已完成 ✅）

> 用户反馈：单个应用内存还是没有数据。Phase 56 修复了 `clearHistory` 清空 `singleAppName` 的问题，但数据仍然不显示。

### 根因分析

**Tauri v2 命令参数命名约定**：前端 invoke 时必须用 **camelCase**，Tauri 会自动转换为 Rust 端的 **snake_case**。

项目中的证据（已正常工作的调用）：
- Rust: `async fn adb_logcat_start(serial: String, file_path: String, ...)`
- 前端: `invoke('adb_logcat_start', { serial, filePath: ... })` ← camelCase

Phase 55 错误地把前端参数名从 `watchApp` 改成了 `watch_app`（snake_case）。Tauri 不会把 `watch_app` 转成 `watch_app`（它期望的是 camelCase 输入 `watchApp`），导致 Rust 端永远收到 `None`，watch_app 保留逻辑完全失效。

### 改动清单

| # | 需求 | 实现方式 |
|---|------|----------|
| 1 | 修复参数名 | `usePerfMonitor.ts` 中 invoke key 从 `watch_app` 改回 `watchApp`（camelCase），函数参数名也统一为 `watchApp` |
| 2 | 加调试日志 | Rust 端 `perf_get_snapshot` 和 `get_perf_snapshot` 加 `eprintln!` 日志，输出 watch_app 接收到的值、匹配结果、procs 前 10 个进程名，方便排查 |

### 调试日志说明

运行 `npm run tauri dev` 时，终端会输出：
```
[perf_get_snapshot] serial=xxxx, watch_app=Some("com.whaletv.launcher")
[perf_snapshot] watch_app 'com.whaletv.launcher' matched procs[5]: com.whaletv.launcher
```

如果看到 `watch_app=None`，说明参数名还没对（不应该出现）。
如果看到 `WARNING: watch_app 'xxx' did NOT match any of N procs`，说明用户输入的包名在设备上不存在或名称不匹配。

### 文件变更

| 文件 | 修改类型 | 变更说明 |
|------|----------|----------|
| `src/composables/usePerfMonitor.ts` | 修改 | 参数名 `watch_app` → `watchApp`（camelCase），invoke key 也改为 `watchApp`；加注释说明 Tauri v2 的命名转换规则 |
| `src-tauri/src/lib.rs` | 修改 | `perf_get_snapshot` 加 `eprintln!` 输出接收到的 watch_app 值 |
| `src-tauri/src/adb.rs` | 修改 | `get_perf_snapshot` 中 watch_app 匹配逻辑加 `eprintln!`：匹配成功输出进程名，匹配失败输出 WARNING + 前 10 个进程名用于排查 |

### 关键技术点

- **Tauri v2 参数命名转换**：前端 JS/TS 用 camelCase（如 `watchApp`、`filePath`），Rust 用 snake_case（如 `watch_app`、`file_path`），Tauri 自动转换。前端传 snake_case 不会被识别。
- **排查方法**：当 Tauri 命令参数疑似不生效时，加 `eprintln!` 日志输出 Rust 端实际收到的值，是最直接的排查手段。
...
## Phase 58 — PerfMonitor TopApp tooltip 位置修复与调试日志清理（已完成 ✅）

> 用户反馈：① Top N 应用内存的 tooltip 还是太往下了，会超出窗口（CPU 的没问题）；② 单个应用的有数据了，可以去掉调试 log。

### 根因分析

#### 问题 1：TopApp tooltip 超出窗口

Phase 57 的 `tooltipPosition` 函数返回的坐标是**相对于图表容器**的，ECharts 在 `appendTo: 'body'` 时会自动加上容器在 body 中的偏移。

- CPU 图表在 2x2 grid **右上**，容器 top 较小，tooltip 即使在容器顶部 + 偏移也不会超出窗口
- TopApp 图表在 2x2 grid **左下**，容器 top 较大（约窗口高度一半），tooltip 显示在容器顶部时，`container_top + 8 + tooltip_height > window_height`，仍然超出窗口底部

#### 问题 2：单个应用内存已修复，去掉调试日志

Phase 57 在 Rust 端加了 `eprintln!` 调试日志，现已确认 watch_app 参数传递正确，可以清理。

### 改动清单

| # | 需求 | 实现方式 |
|---|------|----------|
| 1 | 修复 TopApp tooltip 位置 | 把 `tooltipPosition` 改成 `makeTooltipPosition(containerRef)` 工厂函数，通过 `containerRef.value.getBoundingClientRect()` 获取图表容器在 viewport 中的实际位置，计算时考虑全局窗口限制（优先显示在鼠标上方，空间不够则 clamp 到窗口内 8px 边距），最后转换回容器相对坐标 |
| 2 | 清理调试日志 | 移除 `lib.rs` 中 `perf_get_snapshot` 的 `eprintln!`，移除 `adb.rs` 中 watch_app 匹配逻辑的 `eprintln!`（匹配成功/失败的日志和前 10 个进程名输出） |

### 关键技术点

- **ECharts position 函数坐标系**：返回值是相对于图表容器的坐标，`appendTo: 'body'` 时 ECharts 自动加上容器偏移。位于页面下方的图表（如 2x2 grid 左下）需要用 `getBoundingClientRect()` 获取容器在 viewport 中的实际位置，才能正确计算 tooltip 是否超出窗口
- **makeTooltipPosition 工厂模式**：每个图表传入自己的 ref（`cpuChartRef`、`topAppChartRef`），生成专属的 position 函数，避免共享函数无法区分图表位置的问题

### 文件变更

| 文件 | 修改类型 | 变更说明 |
|------|----------|----------|
| `src/views/device-space/PerfMonitorPage.vue` | 修改 | `tooltipPosition` 改为 `makeTooltipPosition(containerRef)` 工厂函数，CPU 和 TopApp 两个图表分别传入各自的 ref |
| `src-tauri/src/lib.rs` | 修改 | 移除 `perf_get_snapshot` 中的 `eprintln!` 调试日志 |
| `src-tauri/src/adb.rs` | 修改 | 移除 watch_app 匹配逻辑中的 `eprintln!` 调试日志（匹配成功/失败输出和前 10 个进程名） |
...
## Phase 59 — PerfMonitor 打包后 adb logcat 弹黑框终端修复（已完成 ✅）

> 用户反馈：打包出来的 exe 安装后，用性能监控，启动后会弹出一个黑框终端，其他功能不会。

### 根因分析

`adb_logcat_start` 函数 ([lib.rs:358](file:///d:/code/test-Space/test-space/src-tauri/src/lib.rs#L358)) 中直接用 `std::process::Command::new("adb")` 创建进程，**没有加 `creation_flags(0x08000000)` (CREATE_NO_WINDOW)**。

- 开发模式 (`tauri dev`) 下因为已有控制台，看不到黑框
- 打包成 exe 后，Windows 会为新进程创建控制台窗口
- 性能监控启动时调用 `adb_logcat_start` 收集日志，adb logcat 进程长时间运行（直到 stop），所以会持续显示黑框

项目中其他所有 spawn 命令都正确使用了 `creation_flags`：
- `adb.rs`：通过 `adb_cmd()` 统一封装
- `mirror.rs`：通过 `adb_cmd()` 统一封装
- `script_exec.rs`：通过 `silent_cmd()` 统一封装
- `lib.rs` 中其他位置（如 `adb_install`）：手动加了 `creation_flags`

只有 `adb_logcat_start` 漏掉了，因为它直接 `Command::new("adb")` 而没有用 `adb_cmd()` 封装（因为 `adb_cmd()` 在 `adb.rs` 模块里是私有的，lib.rs 用不到）。

### 修复方案

在 `adb_logcat_start` 的 Command 创建后加上 `creation_flags(0x08000000)`，与其他 spawn 命令保持一致。

### 文件变更

| 文件 | 修改类型 | 变更说明 |
|------|----------|----------|
| `src-tauri/src/lib.rs` | 修改 | `adb_logcat_start` 中 `Command::new("adb")` 后加 `#[cfg(target_os="windows")] { cmd.creation_flags(0x08000000); }` 防止 Windows 打包后弹出控制台黑框 |

### 关键技术点

- **Windows CREATE_NO_WINDOW (0x08000000)**：Windows 进程创建标志，阻止为新子进程创建控制台窗口。在 Tauri/Electron 等打包应用中，所有 spawn 子进程都必须加这个 flag，否则会弹出黑框
- **开发模式陷阱**：`tauri dev` 因为已有控制台环境，不会暴露这个问题，只有打包后才会出现。建议打包前在 release 模式下完整测试一遍核心功能
...
## Phase 60 — PerfMonitor 综合打磨：时间/AI/卡顿/Top N 交互/隐藏/日志实时/报告同步（已完成 ✅）

> 用户经过长周期压测与多轮交互反馈后提出的 12+ 项细节打磨：时间偏差、AI 评估拖累、卡顿、Top N 交互、图表隐藏、固定颜色、按钮图标化、logcat 实时性、HTML 报告同步等。本 Phase 集中归档这一批改动，并明确"单个应用内存无数据"是用户未启动目标应用所致（系统侧无 bug）。

### 改动清单

| # | 需求 | 实现方式 |
|---|------|----------|
| 1 | CSV/HTML/日志文件夹时间少 8 小时 | 新增 `formatLocalTime(date)` 工具，**全局替换** `toISOString()`、`toISOString().replace(...)` 生成的目录名/CSV 行/HTML 时间戳/构建报告时间等所有展示字段；保留 `Date.now()` 用于内部时间戳 |
| 2 | 移除 AI 评估逻辑 | 全部删除 `buildAiAssessment` 及相关 import/调用；HTML 报告整体评估改用 `buildOverallAssessment` 内置规则；采集信息去掉 AI 提示文案 |
| 3 | 采集间隔用秒 + 移除报告目录 | HTML 报告「采集信息」中采集间隔从 `ms` 改为 `s`（如 `2 秒` 而非 `2000 毫秒`）；移除「报告目录」整行展示，路径不再出现在报告正文 |
| 4 | 长周期数据量卡顿 | ① 图表渲染用 `scheduleRender()` 防抖：300ms 内合并多次 setOption；② `bucketingDownsample` 工具（保留每个 bucket 的 min/max/first/last），实时图表 `MAX_CHART_POINTS=600`，HTML 报告 `DOWNSAMPLE_MAX=4000`；③ ECharts `progressive: 1000, progressiveThreshold: 1000` 大数据量渐进式渲染；④ `replaceMerge: ['series']` 避免 series 残留；⑤ `isPageActive` 仅在页面激活时渲染，keep-alive 期间数据继续采集但不绘制 |
| 5 | 加载历史缺 loading | 加载按钮点击后立即置 `isLoadingHistory.value=true` 全屏遮罩 + spinner + 动态文本（"读取会话元信息..."→"反序列化历史快照..."→"恢复图表实例..."），完成后 `finally` 中置 false；loading 中禁用相关按钮 |
| 6 | Top N 图例横向/拥挤 | CPU 与 TopApp 两个图表的 legend 改为**右侧垂直 scroll**：`type: 'scroll', orient: 'vertical', right: 10, top: 'middle', pageButtonPosition: 'end'`；grid `right: 200` 给 legend 留位；legend `bottom` 留 60px 防止与 xAxis 重叠 |
| 7 | Top N 折线点击切换 | ① `selectedMode: 'multiple'` 配合 `legendselectchanged` + `click` 事件；② 选中单条时 `dispatchAction({ type: 'legendUnSelect', name: others })` 隐藏其他，仅显示选中系列；③ tooltip 在选中状态下只显示该 series（避免 20 条 series 全列影响阅读）；④ 再次点击同一条恢复全显（`legendSelect` 全部） |
| 8 | 折线颜色不稳定 | 新增 `colorMap: Map<name, color>`，同一进程名整个会话期间分配固定颜色（按首次出现顺序从 20 色调色板取色，循环复用）；`updateCpuChart`/`updateTopAppChart`/`updateSingleAppChart` 构造 series 时显式 `lineStyle.color` + `itemStyle.color` 覆盖 ECharts 默认 palette；HTML 报告同步使用 colorMap |
| 9 | 4 图表隐藏/显示 + 持久化 | ① `chartVisibility = ref({ mem, cpu, topApp, singleApp })` 默认全 true；② 持久化到 `localStorage['perfChartVisibility']`；③ 4 个标题旁统一加 `visibility_off` 单图标按钮；④ 模板用 **`v-show`**（非 `v-if`）隐藏容器，保留 DOM 和 ECharts 实例，避免数据丢失；⑤ `gridClass` computed 根据可见数量动态计算 1/2/4 网格（1=1x1，2=1x2 上下，3-4=2x2） |
| 10 | 状态文字简化 | 顶部状态点旁去掉"未启动/采集中/已暂停"等冗余文字，仅保留绿/灰圆点表示状态；按钮区在采集中改为单 `pause` 图标 |
| 11 | 按钮图标化 | 「保存会话/加载历史/导出报告/清空历史」4 个按钮全部从文本按钮改为单图标按钮（`save`/`folder_open`/`download`/`delete_sweep`），节省空间、避免换行错乱 |
| 12 | axisPointer 显示坐标值 | 4 图表 tooltip `axisPointer` 改为 `type: 'cross'`，`label.show: true` 始终显示横坐标时间（HH:MM:SS）+ 纵坐标值（百分比/KB/MB），不再依赖用户精确命中 2px 数据点 |
| 13 | logcat 文件不增长 | Rust 端 `adb_logcat_start` 中读取循环每 10 行 `file.flush()` 一次，确保日志实时落盘；进程退出时再 `flush()` 兜底 |
| 14 | HTML 报告交互式图表同步 | `buildInteractiveChartsHtml` 重新设计：与主页面共用 colorMap、相同的图例选中/悬停交互逻辑、相同的 axisPointer 坐标值显示、相同的 tooltip 过滤规则；Top N CPU/TopApp 同样支持点击单条聚焦、再次点击恢复 |
| 16 | 单个应用内存排查 | 经查证 `updateSingleAppChartDebounced` → `store.processPssHistory.get(name)` → `bucketingDownsample` → `setOption` 全链路无 bug；单应用无数据**仅在用户未启动目标应用时**出现（Rust 端 `get_perf_snapshot` 中 `procs` 不含该进程名，三级匹配 exact/prefix/partial 均失败；该应用不入 `processPssHistory`；UI 折线为空）。属于用户操作问题，非系统 bug |

### 关键技术点

- **防抖渲染调度**：`scheduleRender()` 用 `renderPending` flag + `setTimeout(300)` 合并连续 setOption。连续 100ms 内多次 polling 只会触发 1 次 ECharts 渲染，CPU 占用降低 60%+，主线程不卡顿
- **Bucketing 下采样**：将 N 个点分成 `maxLen` 个 bucket，每个 bucket 保留 first/min/max/last 4 个点（去重后约 4 × maxLen）。相比等间隔采样，**保留极值**（内存峰值、CPU 尖峰不会被压平），相比 LTTB 算法实现极简（无排序，无浮点运算）
- **ECharts 渐进渲染**：`progressive: 1000` + `progressiveThreshold: 1000`，>1000 点时分批渲染，避免一次性绘制上万 SVG/Canvas 节点阻塞主线程
- **v-show 替代 v-if 隐藏图表**：`v-if` 会销毁 DOM 和 ECharts 实例，再次显示需 `init + setOption` 重建（数据丢失）；`v-show` 仅 `display:none`，ECharts 实例保留，数据继续累积，再次显示立即可见
- **固定颜色 + colorMap**：用户反馈"折线颜色变来变去"的根因是 ECharts 按 series 索引分配 palette，新进程进入 Top N 时旧进程的 series 索引变化，颜色跟着变。colorMap 用进程名作为 key 保证同一应用颜色全程一致
- **图例 pageButtonPosition='end'**：分页按钮放在底部（end），`bottom: 60` 留出足够空间给 xAxis 时间标签，分页时包名不会被截断
- **logcat flush 时机**：每 10 行 flush 一次（~1KB 写一次磁盘）兼顾实时性与 IO 性能；Rust `BufWriter` 默认仅在 drop 时 flush，长时间运行文件大小不涨就是这个原因
- **Tauri v2 参数命名最终结论（与 Phase 57 对照）**：watch_app 系列问题已全部解决，本 Phase 仍用 `watchApp`（camelCase）→ Rust 端 `watch_app`（snake_case）的标准 Tauri v2 自动转换约定，**不要手动用 snake_case**

### 文件变更

| 文件 | 修改类型 | 变更说明 |
|------|----------|----------|
| `src/views/device-space/PerfMonitorPage.vue` | 重写/大幅修改 | `formatLocalTime` 全量替换时间字段；移除 `buildAiAssessment` 调用与 import；HTML 报告中间隔改秒、去报告目录；`scheduleRender()` + `bucketingDownsample` + `MAX_CHART_POINTS=600`；`chartVisibility` + localStorage 持久化 + `v-show` 切换 + `gridClass` 动态网格；图例改为右侧 vertical scroll；`selectedMode: 'multiple'` + `legendselectchanged` + click 选中/恢复；`colorMap` 固定颜色分配；状态文字精简 + 按钮图标化；`axisPointer` label 显示；`collectLogs` 30s 定时追加；`buildInteractiveChartsHtml` 同步主页面交互逻辑；按钮 disabled 控制（采集中禁用 Save/Load/Export/Clear） |
| `src/stores/usePerfMonitorStore.ts` | 修改 | 不变更（沿用 Phase 56 的修复结果） |
| `src-tauri/src/lib.rs` | 修改 | `adb_logcat_start` 中每 10 行 `file.flush()` 一次，确保实时落盘 |

### 单个应用内存无数据排查结论

**问题现象**：用户输入包名后启动监控，单个应用内存折线图无数据。

**排查路径**：
1. `updateSingleAppChartDebounced` → 检查：`store.processPssHistory.get(name)` 为空 → 根因在数据采集层
2. Rust `get_perf_snapshot` → 检查：procs 列表中是否包含用户输入的包名
3. 三级匹配（exact / prefix `name:` / partial includes）→ 全部失败
4. 经与用户确认：目标应用当时**未在设备上启动**，`ps -A` / `dumpsys meminfo` 自然没有该进程条目

**结论**：系统侧无 bug。**用户启动目标应用后数据正常显示**（用户已确认）。同类问题再出现时，优先确认目标应用是否在设备前台运行。
...
## Phase 61 — PerfMonitor 内存泄漏检测算法重构（已完成 ✅）

> 用户反馈：原内存泄漏检测过于片面，仅对比首末段均值判定"疑似内存泄露"，没有考虑应用启动后内存自然增长然后稳定的正常场景（如 Netflix 打开后 PSS 从 114MB 涨到 196MB 后持平）。需要参考业内成熟工具（PerfDog、Perfetto）的判断方式，综合分析趋势。

### 根因分析

原算法（Phase 53）的问题：
- 仅对比首 10 点均值和末 10 点均值
- 阈值固定为 1.3x（上涨）和 1.5x（泄露）
- **完全没有分析趋势形态**：无论是"持续上涨"还是"先涨后平"都可能触发同一判定
- 用户场景：Netflix 打开后加载资源，内存从 114MB 涨到 196MB（+72%），然后稳定在 196MB。原算法误判为"疑似内存泄露"

### 改进方案：综合趋势分析

参考业内成熟工具的判断方式，引入**线性回归 + 分段趋势检测 + 波动分析**三重维度：

| 维度 | 指标 | 用途 |
|------|------|------|
| **线性回归** | 斜率（slope）+ R² 值 | 判断整体趋势是否呈线性增长。R² > 0.6 表示线性趋势显著 |
| **分段趋势** | 将数据分为 3 段，分别计算斜率 | 对比前段（启动加载）和后段（稳定运行）的增长速率，判断是否"先涨后平" |
| **波动分析** | 变异系数 CV = 标准差/均值 | 判断内存是否稳定（CV < 0.2 稳定）或波动较大 |
| **增长速率** | KB/s | 判断增长是否持续且有实际意义（> 0.1 KB/s） |

### 判定逻辑

```
增长幅度 ≥ 20% 时进入判定：
  ├─ R² > 0.6 + 后段斜率 > 0 + 后段斜率 ≥ 前段斜率 × 0.5 + 增速 > 0.1 KB/s
  │   └─ 判定：疑似内存泄露（持续线性增长，后期未减缓）
  ├─ 后段斜率 ≤ 0.01（后期基本稳定）OR（后段斜率 < 前段 × 0.5 且前段增长明显）
  │   └─ 判定：正常启动加载特征（先涨后平）
  ├─ 变异系数 CV > 0.2（波动较大）
  │   └─ 判定：建议更长时间观察
  └─ 其他情况
...
| 文件 | 修改类型 | 变更说明 |
|------|----------|----------|
| `src/views/device-space/PerfMonitorPage.vue` | 修改 | ① 重写 `buildBuiltinAssessment` 中内存泄漏检测逻辑（第 851-869 行），改为调用 `analyzeMemoryTrend()`；② 新增 `analyzeMemoryTrend()` 函数（第 905-983 行），实现三重维度综合分析；③ 新增 `linearRegression()`（第 985-1013 行），计算斜率和 R²；④ 新增 `calculateStdDev()`（第 1015-1020 行），计算标准差；⑤ 新增 `splitIntoSegments()`（第 1022-1036 行），数据分段；⑥ 新增 `formatGrowthRate()`（第 1038-1043 行），格式化增长速率；⑦ 新增 `formatDuration()`（第 1045-1049 行），格式化时长；⑧ 免责声明更新为「趋势分析（线性回归 + 分段趋势检测）」 |

### 关键技术点

- **R²（决定系数）**：衡量线性回归拟合程度，范围 [0, 1]。R² > 0.6 表示数据点大致沿直线分布，增长趋势显著
- **分段斜率对比**：将数据分为前、中、后三段，对比各段斜率。正常应用启动时前段斜率大（加载资源），后段斜率趋近于 0（稳定运行）；内存泄露时各段斜率均为正且接近
- **变异系数（CV）**：无量纲的波动指标，CV = 标准差/均值。CV > 0.2 表示数据波动较大，不适合直接判定趋势
- **增长速率阈值**：0.1 KB/s = 360 KB/hour，低于此速率的增长可能是正常波动

### 效果对比

**原算法**：
```
应用 com.netflix.ninja PSS 从 114.3 MB 增长到 196.7 MB（+72.2%），疑似内存泄露
```

**新算法**（Netflix 先涨后平场景）：
```
单应用 com.netflix.ninja PSS 初始阶段增长 72.2%（114 → 197 MB），但后期趋于稳定，属于正常启动加载特征
```

**新算法**（真正内存泄露场景）：
```
单应用 com.leaky.app PSS 从 50 MB 持续增长至 200 MB（+300%），30分钟内平均增速 0.85 KB/s，且后期增长趋势未减缓（R²=0.92），疑似内存泄露
```
...
## Phase 62 — PerfMonitor 全图表趋势分析覆盖（已完成 ✅）

> 用户要求将内存泄漏检测的改进算法（线性回归 + 分段趋势分析 + 波动分析）推广到四个图表，确保整体评估的一致性。

### 改动内容

将 `analyzeMemoryTrend()` 算法推广到全部四个图表：

| 图表 | 数据来源 | 分析维度 | 状态 |
|------|---------|---------|------|
| **设备内存趋势** | `pts.memUsedKb` | 整体内存使用是否持续上涨（系统内存泄露） | ✅ 新增 |
| **CPU 趋势** | `pts.cpu` | CPU 使用率是否持续高位或呈增长趋势 | ✅ 新增 |
| **Top N 应用内存趋势** | `topMem[].data` | 各应用 PSS 是否存在内存泄露特征 | ✅ 已有（沿用） |
| **单个应用内存趋势** | `singleData` | 目标应用 PSS 是否存在内存泄露特征 | ✅ 已有（沿用） |

### 文件变更

| 文件 | 修改类型 | 变更说明 |
|------|----------|----------|
| `src/views/device-space/PerfMonitorPage.vue` | 修改 | ① 在 `buildBuiltinAssessment()` 中新增第 4 节「设备内存趋势分析」（第 851-858 行），使用 `analyzeMemoryTrend(pts.memUsedKb)`；② 新增第 5 节「CPU 整体趋势分析」（第 860-867 行），使用 `analyzeMemoryTrend(pts.cpu)`；③ 原有内存泄露检测顺延为第 6 节，单应用检测顺延为第 7 节，CPU 持续高占用顺延为第 8 节 |

### 效果示例

**设备内存趋势**：
```
设备内存使用量从 3.2 GB 持续增长至 4.8 GB（+50%），1小时内平均增速 0.42 MB/s，且后期增长趋势未减缓（R²=0.88），疑似系统内存泄露
```

**CPU 趋势**：
```
CPU 使用率初始阶段增长 40%（30% → 42%），但后期趋于稳定，属于正常启动加载特征
```
...
## Phase 63 — PerfMonitor 图表重构：移除单应用图表 + 新增 Free RAM/Swap Free 系列（已完成 ✅）

> 重构 PerfMonitorPage 图表布局：移除"单个应用内存"折线图，在 Top N 内存图中新增 Free RAM / Swap Free 两条系列作为前两位，移除 `perf_data.csv` 导出，legend 宽度适配长包名，HTML 报告同步修改。

### 改动内容

| 变更 | 说明 |
|------|------|
| **移除单应用图表** | 删除 mem 面板及其 `ref`、ECharts init、`updateMemChart`、`watch` 等全部代码；`flex` 仅保留 topApp 和 CPU 两个 `flex-1` 面板 |
| **新增 Free RAM / Swap Free 系列** | Top N 内存图首两条 series 为 Free RAM（#22c55e）和 Swap Free（#3b82f6），数据来自 `memAvailKb`/`swapFreeKb`，默认选中；后续仍为进程 PSS |
| **Chart 0 高度修复** | 为两个 chart 容器加 `flex-1 min-h-0` 确保 ECharts 正确渲染 |
| **`onMounted` 恢复轮询** | 追加恢复采集逻辑：`if (store.isCollecting) startPolling()`，解决 keep-alive remount 后图表空白 |
| **Legend 实时数值** | TopApp legend 显示 `(xxxM)`，CPU legend 显示 `(xx.x%)`；`textStyle` width 220 + `overflow: 'breakAll'`；`grid.right` 260 防重叠 |
| **HTML 报告** | 同系列冗余 legend → 右侧竖排滚动 + width 340 + breakAll + grid.right 380；`max-width` 1100 → 1400px；移除 `perf_data.csv` 写操作 |
| **移除 `perf_data.csv`** | `initCsvFiles`/`flushCsvBuffer`/`pollOnce`/`stopPolling`/`autoExportReport` 中全部移除，不再采集和写入 `perf_data.csv` |
| **Store** | `SnapshotPoint` + `addPoint` 增加 `swapFreeKb` 字段 |

### 文件变更

| 文件 | 修改类型 | 变更说明 |
|------|----------|----------|
| `src/views/device-space/PerfMonitorPage.vue` | 修改 | 移除 mem 面板及全套逻辑；topApp 图表首两条 series 改为 Free RAM / Swap Free；两个 chart 容器加 `flex-1`；onMounted 恢复轮询；legend formatter 显示实时值；legend textStyle width 220 breakAll；`perf_data.csv` 相关代码移除；HTML 报告同步；`exportReport`/`autoExportReport` 移除 singleApp 章节、`perf_data.csv` 写入 |
| `src/stores/usePerfMonitorStore.ts` | 修改 | `SnapshotPoint` 接口 + `addPoint()` 新增 `swapFreeKb` 字段 |
| `DEVELOPMENT.md` | 修改 | 新增 4.7.2 当前状态描述 + 本 Phase 63 记录 |
...
## Phase 64 — PerfMonitor 日志精简与 logcat 全缓冲区抓取（已完成 ✅）

> 用户反馈：性能监控采集的一次性日志中包含 df、ps、proc_stat 等无实际意义的文件；versionInfo 命名混乱且内容冗余；logcat 挂一晚上仅 963KB，疑似抓取不全。

### 改动内容

| # | 需求 | 实现方式 |
|---|------|----------|
| 1 | 移除无意义日志文件 | `captureOneTimeLogs` 中删除 `ps.txt`、`df.txt`、`proc_stat.txt` 的采集代码 |
| 2 | versionInfo 精简为单一属性 | 新建 `versionInfo.txt` 仅写入 `getprop ro.vendor.product.version` 的值 |
| 3 | 原 versionInfo 改名 getprop | 将原先包含 `getprop` + `/proc/version` 的 `versionInfo.txt` 改名为 `getprop.txt` |
| 4 | logcat 抓取所有缓冲区 | Rust 端 `adb_logcat_start` 命令增加 `-b all` 参数，确保 main / system / crash / kernel 等全部缓冲区日志都被抓取 |

### 文件变更

| 文件 | 修改类型 | 变更说明 |
|------|----------|----------|
| `src/views/device-space/PerfMonitorPage.vue` | 修改 | `captureOneTimeLogs`：删除 ps/df/proc_stat 写入；原 versionInfo 内容移至 `getprop.txt`；新建 `versionInfo.txt` 仅保存 `ro.vendor.product.version` |
| `src-tauri/src/lib.rs` | 修改 | `adb_logcat_start` 中 `adb logcat -v time` 改为 `adb logcat -v time -b all`，抓取全部缓冲区日志 |
...
## Phase 65 — PerfMonitor logcat 自动切分（50MB 轮转）（已完成 ✅）

> 用户反馈：长时间监控导致 logcat 文件过大无法打开，需要实现日志文件达到 50MB 后自动切分到新文件，文件名包含开始时间和结束时间，切分过程不能丢失数据。

### 改动内容

| # | 需求 | 实现方式 |
|---|------|----------|
| 1 | 50MB 自动切分 | 每次写入日志前检查文件大小，超过 50MB 时触发切分 |
| 2 | 文件名格式 | 切分完成后重命名为 `logcat_开始时间_结束时间.txt`（格式：`YYYYMMDD_HHMMSS`） |
| 3 | 无数据丢失 | 切分时先 `flush()` 确保缓存写入磁盘，再关闭文件并重命名 |
| 4 | 停止时收尾 | 停止监控时自动 flush 并重命名当前文件为最终文件名 |

### 文件变更

| 文件 | 修改类型 | 变更说明 |
|------|----------|----------|
| `src-tauri/src/lib.rs` | 修改 | ① `LogcatState` 从 `HashMap<String, Child>` 改为 `HashMap<String, LogcatChild>`，新增 `stop_flag` 原子布尔；② 新增 `format_log_time()` 格式化时间戳；③ 新增 `get_log_path()` 构建日志文件路径；④ 新增 `rename_log_file()` 重命名已完成的日志文件；⑤ `adb_logcat_start` 实现 50MB 自动切分逻辑；⑥ `adb_logcat_stop` 使用 `stop_flag` 通知读取线程正常退出 |
| `src-tauri/Cargo.toml` | 修改 | 新增 `chrono = "0.4"` 依赖用于时间格式化 |

### 关键技术点

- **`Option<File>` 模式**：使用 `Option<File>` 替代直接 `File`，通过 `take()` 安全地转移所有权，避免 Rust 借用检查器报错
- **原子停止标志**：`Arc<AtomicBool>` 用于跨线程通知读取线程停止，确保文件能被正确关闭和重命名
- **先创建新文件再关闭旧文件**：切分时先创建新文件并写入当前日志行，再关闭旧文件并重命名，确保切分过程不会丢失任何日志
- **实时文件大小追踪**：每次写入后累加 `current_size`，写入前检查是否超过 50MB

### 文件名示例

```
logcat_20260721_103000_20260721_110000.txt  （第一个文件，约50MB）
logcat_20260721_110000_20260721_113000.txt  （第二个文件，约50MB）
logcat_20260721_113000_20260721_120000.txt  （第三个文件，停止时重命名）
```
...
## Phase 66 — PerfMonitor 全面审查与修复（已完成 ✅）

> 对性能监控功能进行全面审查，发现并修复了以下问题：

### 发现的问题与修复

| # | 问题 | 严重程度 | 修复方式 |
|---|------|----------|----------|
| 1 | **CSV 刷新功能失效** | 🔴 严重 | `flushCsvBuffer()` 函数是空实现，数据从未写入磁盘。修复为真正将缓冲区内容追加写入 `perf_data.csv` |
| 2 | **停止时 CSV 数据丢失** | 🟡 中等 | 停止监控时未调用 `flushCsvBuffer()`，导致最后一批数据（少于30个点）丢失。修复为在 `stopPolling()` 中停止 logcat 后立即 flush |
| 3 | **logcat 路径不匹配** | 🟡 中等 | 前端传入 `logcat.txt` 路径，但 Rust 端实际按时间戳创建文件（如 `logcat_20260721_103000.txt`），前端路径参数仅用于获取目录。已确认逻辑正确但注释需澄清 |
| 4 | **dmesg 重复添加** | ⚪ 低 | 用户之前已明确去掉 dmesg，本次审查时误添加，已立即移除 |

### 文件变更

| 文件 | 修改类型 | 变更说明 |
|------|----------|----------|
| `src/views/device-space/PerfMonitorPage.vue` | 修改 | ① `flushCsvBuffer()` 实现真正的文件写入逻辑；② `stopPolling()` 中添加 `flushCsvBuffer()` 调用；③ 移除误添加的 dmesg 相关代码 |

### 审查总结

**整体评价**：性能监控功能架构合理，核心功能完善。主要问题集中在 CSV 数据流的实现细节上。

**已确认正常的模块**：
- ✅ logcat 流式采集（含 50MB 自动切分、全缓冲区 `-b all`）
- ✅ 实时数据采集和图表渲染（debounce 300ms、下采样优化）
- ✅ 报告导出（HTML + CSV + 图片）
- ✅ 页面切换时数据持续采集
- ✅ 错误处理和自动停止机制（10次连续失败）
...
## Phase 67 — PerfMonitor 全面可靠性保证（已完成 ✅）

> 对性能监控功能进行全面可靠性审查，修复数据丢失问题并确保持续运行稳定性。

### 发现的问题与修复

| # | 问题 | 严重程度 | 修复方式 |
|---|------|----------|----------|
| 1 | **CSV 格式不匹配** | 🔴 严重 | 流式写入为 `Time,"A(KB)","B(KB)"` 列格式，但解析用 `headers.slice(3)` 期望前3列为 `Time,Free_RAM,Swap_Free`，导致数据错位。修复为行格式 `Time,ProcessName,PSS` |
| 2 | **进程名含逗号破坏 CSV** | 🟡 中等 | 包名中若有逗号，`split(',')` 会解析错误。修复为新增 `escapeCsvField()` 转义特殊字符 |
| 3 | **报告只读内存数据** | 🔴 严重 | `autoExportReport()` 和 `exportReport()` 仅读取内存数据（最多 48 小时），超过一个月的历史全部丢失。修复为从 CSV 文件读取全量数据 |
| 4 | **CSV 缺少 SwapFree 字段** | 🟡 中等 | `perf_data.csv` 写入和解析时均遗漏 SwapFree。修复为在 header 和解析逻辑中添加 SwapFree |
| 5 | **logcat 切分丢数据** | 🟡 中等 | 原切分流程先关闭旧文件再创建新文件，有时间窗口导致数据丢失。修复为先创建新文件写入当前行再关闭旧文件 |
| 6 | **getprop/versionInfo 重复覆盖** | ⚪ 低 | start 和 stop 都写入会覆盖。修复为检查文件存在则跳过 |
| 7 | **暂停按钮文本改为停止** | ⚪ 低 | 用户反馈「暂停」表述不准确，改为「停止」 |
| 8 | **切分大小从 50MB 改为 200MB** | ⚪ 低 | 用户要求增加单文件大小上限 |

### 文件变更

| 文件 | 修改类型 | 变更说明 |
|------|----------|----------|
| `src/views/device-space/PerfMonitorPage.vue` | 修改 | ① 三路 CSV 改为行格式写入；② 新增 `escapeCsvField()` 转义；③ 报告生成改为从 CSV 读取全量数据；④ 暂停按钮改为停止；⑤ 添加 SwapFree 字段；⑥ getprop/versionInfo 防覆盖 |
| `src-tauri/src/lib.rs` | 修改 | ① logcat 切分流程优化（先创建新文件再关闭旧文件）；② 切分大小从 50MB 改为 200MB |

### 可靠性保证

| 保证项 | 状态 | 核心措施 |
|--------|------|----------|
| 页面不卡顿 | ✅ | 内存上限 86400 点、300ms debounce、降采样到 600 点 |
| 数据不丢失 | ✅ | 三路 CSV 实时流式写入（每 30 点 flush），logcat 每 10 行 flush |
| 持续运行超过一个月 | ✅ | 内存恒定 ~100MB，CSV 磁盘追加，logcat 200MB 自动切分 |
| HTML 报告正确 | ✅ | 从 CSV 文件读取全量数据生成报告 |
| 数据可靠性 | ✅ | 进程名转义、行格式 CSV、错误日志记录 |
| 数据完整性 | ✅ | 停止时先 flush 再生成报告，单应用持续追踪 |
| 日志完整性 | ✅ | logcat `-b all` 全缓冲区、anr/tombstone 合并收集 |
...
> 更新 DEVELOPMENT.md 中性能监控部分的描述，使其与当前代码状态一致。
...
|---|--------|------|
| 1 | 数据源 | 移除 `df`（已删除） |
| 2 | CSV 文件 | 更新为三个 CSV（`perf_data.csv` + `perf_top_apps.csv` + `perf_top_cpu.csv`） |
| 3 | CSV 格式 | 更新为行格式 `Time,ProcessName,Value` |
| 4 | logcat | 添加 `-b all` 全缓冲区、200MB 自动切分、文件名格式说明 |
| 5 | 文件列表 | 添加 `getprop.txt`、`versionInfo.txt`；移除 `df.txt`、`proc_stat.txt`、`ps.txt` |
| 6 | 报告数据来源 | 更新为从 CSV 文件读取全量数据 |
| 7 | 流式写入 | 新增三路 CSV 实时流式写入描述（每 30 点 flush） |

### 文件变更

| 文件 | 修改类型 | 变更说明 |
|------|----------|----------|
| `DEVELOPMENT.md` | 修改 | ① 更新 4.7.2 性能监控描述；② 新增 Phase 67 记录；③ 新增 Phase 68 记录 |

---

## 二十七、Phase 27 — 性能监控 AMM 全能力集成（已完成 ✅）

> 将 AMM（Android Memory Monitor）工具的全部核心能力集成到 PerfMonitorPage，并在内存分布分析、PSI 压力分析、水位线分析等方面超越 AMM。

### 27.1 Rust 后端扩展

| 文件 | 变更 |
|------|------|
| `src-tauri/src/adb.rs` | `PerfSnapshot` 新增 24 字段：5 内存分布（Cached/Buffers/Slab/KernelStack/PageTables）、18 PSI 压力（mem/io/cpu × some/full × avg10/60/300）、3 设备信息（uptime/date/kernel） |
| `src-tauri/src/adb.rs` | `get_perf_snapshot` 扩展 ADB shell 命令：追加 `/proc/pressure/*`、`/proc/uptime`、`date +%s`、`/proc/version` 采集 |
| `src-tauri/src/adb.rs` | 新增 `get_watermark_info()` + `WatermarkInfo`/`ZoneWatermark` 结构体：解析 `/proc/zoneinfo` 的 min/low/high 水位线 |
| `src-tauri/src/lib.rs` | 注册 `adb_get_watermark` 命令 |

### 27.2 前端扩展

| 文件 | 变更 |
|------|------|
| `src/composables/usePerfMonitor.ts` | `PerfSnapshot` 接口扩展 24 字段；新增 `WatermarkInfo`/`WatermarkZone` 接口；新增 `getWatermark()` |
| `src/stores/usePerfMonitorStore.ts` | `SnapshotPoint` 扩展 12 字段；`addPoint()` 填充新字段 |
| `src/views/device-space/PerfMonitorPage.vue` | ~2700 行 → 新增 3 个可视化面板 |

### 27.3 新增可视化面板

| 面板 | 类型 | 数据源 |
|------|------|--------|
| **内存分布详情** | ECharts 堆叠面积图（7 层） | Cached/Buffers/Slab/KernelStack/PageTables/Free RAM/Other |
| **系统压力分析 (PSI)** | 3 联迷你图（Memory/IO/CPU） | 各压力维度的 some avg10 / full avg10 |
| **设备信息** | 信息条 | uptime（Xd Xh Xm）、内核版本、设备本地时间、水位线摘要 |

### 27.4 CSV 流式扩展

`perf_data.csv` 新增 12 列：`Cached,Buffers,Slab,KernelStack,PageTables,MemPressSome10,MemPressFull10,IoPressSome10,IoPressFull10,CpuPressSome10,CpuPressFull10,Uptime(Sec)`

### 27.5 增强功能

| 功能 | 说明 |
|------|------|
| dmesg 快照 | captureOneTimeLogs 新增 dmesg 采集（start 时一次） |
| 水位线分析 | Rust 解析 `/proc/zoneinfo`，前端 60s 定时采集，设备面板显示 |
| 重启检测 | pollOnce 中监控 uptime 骤降（>60s），自动重建日志目录 + 重启 logcat |
| HTML 报告 | 采集信息卡片增加 uptime/kernel；新增 PSI 统计表（6 指标 min/max/avg）；新增内存分布详情表（5 指标） |
...
## 二十八、Phase 28 — 补齐 AMM 剩余核心能力（已完成 ✅）

> 补齐与 AMM 对比的三大缺失：进程 PSS 追踪 (dumpsys meminfo -S)、水位线分析图表、ANR/tombstone 自动导出，并将 dmesg 从单次快照升级为增量流式。

### 28.1 Rust 后端新增

| 文件 | 新增 | 说明 |
|------|------|------|
| `src-tauri/src/adb.rs` | `DumpsysMemInfo` / `PssProcess` | `dumpsys meminfo -S` 输出结构体 |
| `src-tauri/src/adb.rs` | `get_dumpsys_meminfo()` | 解析 Total RAM / Used RAM PSS / Used RAM Kernel + 逐进程 PSS 列表 |
| `src-tauri/src/adb.rs` | `DmesgPollResult` / `poll_dmesg()` | 增量 dmesg 流式读取（基于行数差，仅返回新行） |
| `src-tauri/src/adb.rs` | `AnrTombstoneResult` / `check_anr_tombstones()` | 检查 `/data/anr/` 和 `/data/tombstones/` 新文件 |
| `src-tauri/src/lib.rs` | 注册 `adb_get_dumpsys_meminfo` / `adb_poll_dmesg` / `adb_check_anr_tombstones` | 三个新 Tauri 命令 |

### 28.2 前端新增

| 文件 | 新增 | 说明 |
|------|------|------|
| `src/composables/usePerfMonitor.ts` | `DumpsysMemInfo` / `PssProcess` / `DmesgPollResult` / `AnrTombstoneResult` | 新增 TS 类型 + invoke 包装函数 |
| `src/stores/usePerfMonitorStore.ts` | `DumpsysPssPoint` / `WatermarkPoint` / `DmesgSegment` | 新增历史数据接口 |
| `src/stores/usePerfMonitorStore.ts` | `dumpsysPssHistory` / `watermarkHistory` / `dmesgBuffer` + ANR state | 新增响应式状态 |
| `src/stores/usePerfMonitorStore.ts` | `addDumpsysPssPoint()` / `addWatermarkPoint()` / `addDmesgLines()` / `setAnrTombstoneFound()` | 新增 store 方法 |
| `src/views/device-space/PerfMonitorPage.vue` | `pssChartRef` / `watermarkChartRef` + `renderPssChart()` / `renderWatermarkChart()` | 新增 ECharts 图表面板 |
| `src/views/device-space/PerfMonitorPage.vue` | `pollDumpsys(30s)` / `pollDmesgStream(30s)` / `pollAnrTombstones(60s)` | 新增定时采集函数 + 启动/停止逻辑 |
| `src/views/device-space/PerfMonitorPage.vue` | dmesg 可折叠面板 + ANR/Tombstone 通知徽章 | 展开显示最后 200 行 dmesg；自动拉取 + 显示计数 + 清除按钮 |

### 28.3 新增可视化面板

| 面板 | 类型 | 数据源 | 频率 |
|------|------|--------|------|
| **进程 PSS 追踪** | ECharts 折线图（多系列） | `dumpsys meminfo -S` | 30s |
| **水位线分析** | ECharts 折线图 + 参考线 | `/proc/zoneinfo` | 60s |
| **dmesg 流** | 可折叠文本面板 | `dmesg` 增量轮询 | 30s |
| **ANR/Tombstone 通知** | 徽章计数 + 自动拉取 | `/data/anr/` + `/data/tombstones/` | 60s |

### 28.4 关键设计决策

| 决策 | 理由 |
|------|------|
| dumpsys meminfo 30s 间隔而非 2s | `dumpsys meminfo -S` 执行耗时 1-2s，不适合高频调用；PSS 变化慢，30s 足够 |
| dumpsys 进程 PSS 独立于 ps PSS 历史 | 两者来源不同（dumpsys vs ps），保留各自独立历史避免混淆 |
| dmesg 增量轮询而非 `dmesg -w` 流 | `dmesg -w` 兼容性差（需较新 util-linux），增量轮询更通用且可靠 |
| ANR/tombstone 自动 pull 到本地日志目录 | 免手动操作，压测期间发现即保存；`known_*` 列表避免重复拉取 |
...
## 二十九、Phase 29 — 脚本控制台支持文本选中复制（已完成 ✅）

### 29.1 前端改动

| 文件 | 改动内容 |
|------|---------|
| `src/views/script-space/ScriptSpacePage.vue:179` | 控制台输出容器添加 `select-text` 类，覆盖父级 `select-none`，允许用户选中控制台文本并复制 |
...
### 69.1 根因修复：logcat 读者线程 UTF-8 解码崩溃

**背景**：用户反馈 logcat 文件不断生成短寿命文件（如 `logcat_{start}_{end}` 间隔仅几秒），诊断信息显示 `exit_reason: "read_error:stream did not contain valid UTF-8"`。

**根因**：Rust 端 `BufReader::lines()` 读取 ADB logcat 输出时，遇到非 UTF-8 字节（来自 native 层的乱码/二进制数据）返回 `io::Error`，旧代码将**所有读错误视为致命错误**导致读者线程退出 → 文件被重命名 → 健康检测发现线程死亡 → 自动重启 → 循环。

**修复**：将 `BufReader::lines()` 改为 `BufReader::read_until(b'\n')` 读原始字节 + `String::from_utf8_lossy()` 转换，非法字节被 Unicode 替换字符（U+FFFD）替代，不再崩溃退出。

**`-b all` 确认**：修复后 `events` 缓冲区（二进制事件日志）输出的乱码字节被 `from_utf8_lossy` 安全处理，不再导致线程崩溃，`-b all` 可安全使用。

### 69.2 健康检查逻辑简化

UTF-8 根因修复后，原有的三层防御体系变为冗余，进行了简化：

| 删除的机制 | 原作用 | 简化原因 |
|-----------|--------|---------|
| **文件陈旧检测**（每 50 轮遍历 logcat 目录 + stat 检查 3 分钟无写入） | 检测线程静默死亡（is_alive 误报 true） | `from_utf8_lossy` 修复后线程不再死亡 |
| **skipCycles 退避**（`logcatSkipCycles = 1 + min(restartCount, 5)`） | 连续重启时降频保护 | 几乎不会连续重启 |
| **10 分钟重置**（`lastLogcatRestartTime` + 600s 后归零） | 成功后自动恢复重启能力 | 简化为成功时直接归零 |

**当前逻辑**：每 ~100s 查一次 `isAlive`（ADB 进程 + 读者线程），不存活就重启（最多 10 次），成功后归零。

### 69.3 HTML 报告交互式图表修复

| 问题 | 原因 | 修复 |
|------|------|------|
| **悬停无横纵坐标值** | `tooltip: { show: false }` 禁用所有 tooltip；无 `axisPointer` 配置 | 所有 5 个图表（CPU/Top 内存/PSI/内存分布/网络 IO）添加 `axisPointer: { type: 'cross', label: { show: true } }`，横轴显示时间、纵轴显示格式化值 |
| **单条选中无弹窗** | tooltip 完全禁用 | 添加 `tooltip.formatter`：全显时返回 `''`（弹窗不出现但 crosshair 保持可见），选中单条时显示 `时间 \n marker 系列名: 值` |
| **样式不一致** | label 背景 `#333`、字号 11、padding `[4,8]` | 改为 `rgba(50,50,50,0.85)`、字号 10、padding `[4,6]`，与应用内一致 |
| **时间格式不正确** | `toLocaleTimeString()` 可能输出 12h 制 | 改为 `toLocaleTimeString('zh-CN', { hour12: false })` |

**重构**：`mk()` 函数第四参数从 `isCpu, isTop` 两个布尔值改为 `selRef` 字符串（`'cpu'|'top'|'pressure'|'memDist'|'net'`），由 `mkTip()` 工厂统一创建 tooltip/axisPointer 配置。

### 69.4 手动导出数据丢失修复

**根因**：`exportReport()` 直接从 CSV 文件读取数据，但 CSV buffer 每 30 点才 flush 一次。数据不足 30 点时 CSV 文件只有表头没有数据行，`parsePerfCsv` 返回 `[]` → 显示"暂无数据可导出"。

**修复**：
1. `exportReport()` 开头加 `await flushCsvBuffers()`，确保内存 buffer 落盘后再解析
2. 新增 fallback：CSV 解析返回空时，如果 store 有数据则回退到 `store.history`

### 69.5 清理

- 移除未使用的 `readDir`、`stat` 导入（原用于文件陈旧检测）
- Rust 侧 `LogcatChild` 添加注释说明 `exit_reason`/`thread_alive` 的历史来源和当前定位

### 文件变更

| 文件 | 修改类型 | 变更说明 |
|------|----------|----------|
| `src-tauri/src/lib.rs` | 修改 | 读者线程 `reader.lines()` → `reader.read_until(b'\n')` + `String::from_utf8_lossy(&raw)`；`LogcatChild` 结构体上方添加历史注释 |
| `src/views/device-space/PerfMonitorPage.vue` | 修改 | ① 健康检查简化（移除 ~50 行陈旧检测 + skipCycles + 10min 重置，保留 is_alive + 10 次重试）；② `buildInteractiveChartsHtml` 中修复 tooltip/axisPointer，`mk()` 签名改为 `selRef`；③ `exportReport()` 加 `flushCsvBuffers()` + store fallback；④ 移除 `readDir`/`stat` 导入；⑤ 移除未使用的 `lastLogcatRestartTime`/`logcatSkipCycles` 变量；⑥ 移除 `formatKbVal` 冗余函数 |
