...
- **布局修复说明（Phase 25）**：修复编辑区无法滚动/卡片溢出的问题。根容器从 `h-full` 改为 `flex flex-1 min-h-0 overflow-hidden`，依赖 `<main>` 的 `flex flex-col` 上下文使 flex-1 正确撑满高度（替代 `h-full` 百分比方案）
  ...
  - **影响范围验证**：`<main>` 添加 `flex flex-col` 后，所有子路由页面变为 flex 项。经验证，`h-full` 在 flex 列上下文中正确解析为 `<main>` 的计算高度，各页面（DeviceSpace、Settings、CaseSpace、CaseEditor、FieldRules、ScriptSpace、ApiSpace、Workspace）无布局冲突
