...
| `/auto-space` | `AutoSpacePage.vue` | **自动化空间**（Phase 2 新增）— 用例管理 + YAML 编辑器 + 执行控制台 + 报告 |
...
- 主内容区 `<main>`：`px-margin-page pt-3 box-border flex-1 overflow-y-auto overflow-x-hidden min-h-0 flex flex-col`
  ...
  - `min-h-0`：允许 flex 项收缩
- **`<keep-alive>`**：`router-view` 包裹 `keep-alive`，缓存 `NotesSpacePage` / `ScriptSpacePage` / `DeviceSpacePage` / `ApiSpacePage` / `SettingsPage` / `CaseSpacePage` / `AutoSpacePage`，切换路由时组件不销毁重建，消除切换卡顿感。每个被缓存的页面组件必须通过 `defineOptions({ name: 'ComponentName' })` 声明 name 以匹配 keep-alive 的 `:include` 列表。
...
**导航项**（按用户使用频率排序）：
Device → Notes → API → Scripts → 【自动化】 → Case → Settings（Settings 固定在右侧）

**自动化导航**：图标 `smart_toy`，位于 Scripts 和 Case 之间，活跃态使用 `glass-active` + FILL 1，`isActive` 匹配逻辑使用 `startsWith` 以支持子路由。
...
### 4.10 Auto Space (`src/views/auto-space/AutoSpacePage.vue`)

**实现（Phase 2 — Test Space 集成）**：
- **路由**：`/auto-space`，组件名 `AutoSpacePage`，被 `keep-alive` 缓存
- **顶部工具栏**：
  - 左侧：当前连接设备显示 + 运行控制（运行选中 ▸ / 运行全部 ▸▸ / 停止 ■）
  - 右侧：新建用例 / 导入 YAML / 报告历史
- **左侧面板（240px, 可折叠）**：
  - 搜索框 + 用例列表按 tags 自动分组显示
  - 每条用例：复选框（多选）+ 名称 + 优先级徽章
  - 每组可折叠/展开，底部 [+ 新建用例]
- **右侧工作区（Tab 切换）**：
  - **Tab 1 — 编辑器**：
    - 用例名可编辑输入框 + 保存状态指示（绿点"已保存"/灰脉冲"保存中..."）
    - YAML 语法高亮编辑器（textarea 覆盖 + 纯 CSS 高亮，类 ScriptSpace 模式）
    - 底部工具栏：保存 / 导出 YAML / 验证 YAML / 运行
    - `Ctrl+S` / `Cmd+S` 快捷键保存
    - 自动保存：编辑内容或名称变化后 1.5s 防抖写入 `auto_cases.yaml_content`
  - **Tab 2 — 执行控制台**：
    - 实时步骤日志输出，状态图标（▶ / ✅ / 🔧 / ❌）+ 文字
    - 清空按钮
  - **Tab 3 — 报告历史**：
    - 历史执行记录列表（时间倒序）
    - 每条显示时间 + 通过/失败/AI修复/跳过数
    - [查看报告] 按钮
- **新建用例对话框**（Teleport 浮层）：
  - 用例名称输入 + 标签输入（逗号分隔）+ 优先级选择（P0/P1/P2/P3）
  - 创建后自动生成 YAML 骨架（包含 meta/setup/一个示例步骤/teardown）
- **导入 YAML**：`@tauri-apps/plugin-dialog` 文件选择器，支持批量 .yaml/.yml
- **导出 YAML**：当前用例 YAML 文本保存到本地文件
- **删除用例**：Teleport 二次确认弹窗
- **数据来源**：`auto_cases` / `auto_run_records` SQLite 表

### 4.11 自动化数据库设计

**新增表（在 `migrateInternal` 中注册）**：

| 表名 | 说明 |
|------|------|
| `auto_cases` | 用例元数据（id/name/file_key/tags/priority/yaml_content 等） |
| `auto_case_versions` | 版本历史（case_id/version/yaml_content/saved_at） |
| `auto_run_records` | 执行记录（run_id/设备/状态/通过数/耗时/报告路径） |
| `auto_run_steps` | 步骤级别执行记录（含截图路径/修复日志） |
| `auto_state_graphs` | StateGraph 版本存档（Phase 5+ 使用） |

**备份恢复**：新增表已纳入 `AppBackup` 类型定义、`exportAllData()` / `importAllData()` / `importAllDataIncremental()` / `validateBackup()` 流程。

### 4.12 自动化 Rust 后端 (`src-tauri/src/auto_runner.rs`)

**命令注册**：
- `auto_run_case` — 单个用例执行（spawn Python 子进程）
- `auto_run_suite` — 批量用例执行
- `auto_stop_run` — 终止执行
- `auto_check_engine` — 检查 Python 引擎可用性

**通信协议**：Python 子进程 stdout 输出 JSON Lines，每行一个事件：
```json
{"type": "step_start", "step_id": "step_01", "desc": "导航到电影频道"}
{"type": "step_done", "step_id": "step_01", "status": "passed", "ms": 320}
{"type": "suite_done", "passed": 9, "failed": 2}
```
Rust 解析每行 JSON 后通过 `tauri::Emitter::emit("auto:step_event", payload)` 推送到前端。前端通过 `listen("auto:step_event")` 接收并实时更新控制台。

### 4.13 自动化前端服务 (`src/services/autoRunService.ts`)

- `runAutoCase(caseId, deviceSerial, onEvent)` — 触发单个用例执行
- `runAutoSuite(caseIds, deviceSerial, onEvent)` — 触发批量执行
- `stopAutoRun(runId)` — 停止运行
- `listenAutoEvents(onEvent)` — 监听实时步骤事件
- `checkPythonEngine()` — 检查 Python 环境
