<template>
  <div class="mobile-page plans-wrap">
    <div class="top-sticky">
      <div class="search-row">
        <input v-model="keyword" :placeholder="t('testplan.searchPlaceholder')" />
      </div>

      <div class="chips">
        <button :class="{ active: statusFilter === '' }" @click="statusFilter = ''">{{ t('report.statusAll') }}</button>
        <button :class="{ active: statusFilter === 'PENDING' }" @click="statusFilter = 'PENDING'">{{ t('testplan.statusPending') }}</button>
        <button :class="{ active: statusFilter === 'IN_PROGRESS' }" @click="statusFilter = 'IN_PROGRESS'">{{ t('testplan.statusInProgress') }}</button>
        <button :class="{ active: statusFilter === 'IN_REVIEW' }" @click="statusFilter = 'IN_REVIEW'">{{ t('testplan.statusInReview') }}</button>
        <button :class="{ active: statusFilter === 'COMPLETED' }" @click="statusFilter = 'COMPLETED'">{{ t('testplan.statusCompleted') }}</button>
      </div>
    </div>

    <section class="list">
      <article v-if="loading" v-for="n in 4" :key="'plan-skeleton-' + n" class="mobile-card card skeleton-card">
        ...
        >
          <div class="head">
            ...
            <div class="metric-box">
              ...
              <strong class="rate">{{ calculatePassRate(row) }}%</strong>
            </div>
          </div>
          <div class="meta">
            <div class="progress-focus">
              <div class="progress-head">
                ...
                <strong class="progress-value">{{ progress(row) }}%</strong>
              </div>
              <div class="progress-track">
                <div class="progress-fill" :style="{ width: `${progress(row)}%` }"></div>
              </div>
            </div>
            <span>执行人：{{ row.executor_names && row.executor_names.length ? row.executor_names.join(', ') : '-' }}</span>
            <span>周期：{{ executionDays(row) }}</span>
          </div>
        </article>
      </TransitionGroup>
    </section>
...
const filteredList = computed(() => {
  ...
  return allData.value.filter(row => {
    ...
    if (!k) return true
    return `${row.name || ''} ${(row.executor_names || []).join(' ')}`.toLowerCase().includes(k)
  })
})

const progress = (row) => (row.total_testcases > 0 ? Math.round(((row.executed_testcases || 0) / row.total_testcases) * 100) : 0)
const calculatePassRate = (row) => {
  const validTestcases = (row.total_testcases || 0) - (row.na_testcases || 0) - (row.nt_testcases || 0)
  if (validTestcases <= 0) return '0.00'
  return (((row.passed_testcases || 0) / validTestcases) * 100).toFixed(2)
}
const executionDays = (row) => {
  ...
  return `${fmt(start)} ~ ${fmt(end)}（${workDays}${t('testplan.days')}）`
}
const statusText = (status) => {
  if (status === 'PENDING') return t('testplan.statusPending')
  if (status === 'IN_PROGRESS') return t('testplan.statusInProgress')
  if (status === 'IN_REVIEW') return t('testplan.statusInReview')
  if (status === 'COMPLETED') return t('testplan.statusCompleted')
  if (status === 'REJECTED') return t('testplan.statusRejected')
  return t('testplan.statusCancelled')
}
const statusClass = (status) => {
  if (status === 'COMPLETED') return 'ok'
  if (status === 'IN_PROGRESS' || status === 'IN_REVIEW') return 'warn'
...
.meta { display:flex; flex-direction: column; gap: 5px; margin: 14px 0 0; color: #5e6066; font-size: 13px; }
.progress-focus {
  margin-bottom: 6px;
  padding: 10px 12px;
  border-radius: 12px;
  background: linear-gradient(135deg, rgba(10, 102, 199, 0.12), rgba(74, 144, 255, 0.1));
}
.progress-head {
  display: flex;
...
.progress-track {
  ...
  height: 8px;
  border-radius: 999px;
  background: rgba(10, 102, 199, 0.14);
  overflow: hidden;
}
.progress-fill {
  height: 100%;
  border-radius: inherit;
  background: linear-gradient(90deg, #0a66c7, #4a90ff);
